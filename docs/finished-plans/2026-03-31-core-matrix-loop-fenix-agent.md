# Core Matrix Loop With Fenix Agent Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Move the provider-backed repeated agent loop into `core_matrix`, keep
`agents/fenix` as the agent layer that prepares prompts and executes
agent-owned tools, and preserve the real 2048 browser capstone as the final
acceptance gate.
...
agent tools must be materialized as workflow-node-scoped durable records so
...
- `Fenix` manifest advertises mailbox-first agent participation
- direct `Fenix` runtime callback routes for `prepare_round` and
  `execute_tool` were removed
- `Fenix` mailbox execution now handles `agent_request` work for both
  `prepare_round` and `execute_tool`
- `Core Matrix` direct `FenixProgramClient` bridge was deleted
- `Core Matrix` now uses `ProviderExecution::AgentRequestExchange`
- `Core Matrix` now persists and routes `agent_request` mailbox items
  and terminal `agent_completed` / `agent_failed` reports
- provider round execution, tool routing, and workflow execution tests were
  updated to the new `agent_request_exchange` collaboration surface
...
assert_equal ["prepare_round", "execute_tool"], manifest.dig("program_contract", "methods")
...
Update the manifest to advertise the mailbox-first agent contract explicitly
...
## Task 2: Add Durable Core Matrix Request-Reply Coordination For Agent Work

**Files:**
- Create: `core_matrix/app/services/agents/request_round_preparation.rb`
- Create: `core_matrix/app/services/agents/request_program_tool_execution.rb`
- Create: `core_matrix/app/services/agents/await_mailbox_report.rb`
- Create: `core_matrix/app/services/agents/program_mailbox_payloads.rb`
...
- Test: `core_matrix/test/services/agents/request_round_preparation_test.rb`
- Test: `core_matrix/test/services/agents/request_program_tool_execution_test.rb`
- Test: `core_matrix/test/services/agents/await_mailbox_report_test.rb`
...
Run: `cd core_matrix && bin/rails test test/services/agents/request_round_preparation_test.rb test/services/agents/request_program_tool_execution_test.rb test/services/agents/await_mailbox_report_test.rb`
...
- create mailbox items for `prepare_round` and `execute_tool`
...
report containing prepared messages and a round-local agent tool catalog.
...
messages plus visible agent tools.
...
- Create: `core_matrix/app/services/provider_execution/prepare_agent_round.rb`
...
Run: `cd core_matrix && bin/rails test test/services/provider_execution/prepare_agent_round_test.rb test/services/provider_execution/route_tool_call_test.rb test/services/tool_bindings/freeze_for_workflow_node_test.rb`
...
```bash
git add core_matrix/app/services/provider_execution/prepare_agent_round.rb core_matrix/app/services/provider_execution/materialize_round_tools.rb core_matrix/app/services/provider_execution/route_tool_call.rb core_matrix/app/services/runtime_capabilities/compose_effective_tool_catalog.rb core_matrix/app/services/tool_bindings/freeze_for_workflow_node.rb core_matrix/test/services/provider_execution/prepare_agent_round_test.rb core_matrix/test/services/provider_execution/route_tool_call_test.rb core_matrix/test/services/tool_bindings/freeze_for_workflow_node_test.rb
...
Cover one successful `prepare_round`, one agent-owned tool execution, one
...
- merge agent-owned tools with kernel-native and MCP tools
- call the provider
- route agent-owned tool calls back through mailbox execution
...
git commit -m "feat: run provider loop through mailbox-backed agent participation"
```
...
Run: `cd core_matrix && bin/rails test test/integration/external_fenix_pairing_flow_test.rb test/services/provider_execution/prepare_agent_round_test.rb`
...
git commit -m "refactor: remove rejected http agent contract"
```
...
- mailbox items and reports for at least one agent-owned tool execution
...
- `Fenix` still owns prompt policy, skills, and agent-owned tools
