# Core Matrix Loop With Fenix Agent Design
...
- `Core Matrix` no longer uses a synchronous direct `Fenix` HTTP client for
  `prepare_round` or `execute_tool`.
- `Core Matrix` creates durable `agent_request` mailbox items and
  waits for terminal control-plane reports through
  `ProviderExecution::AgentRequestExchange`.
- `Core Matrix` accepts `agent_completed` and `agent_failed`
...
- `Fenix` now handles both `prepare_round` and `execute_tool` as
  mailbox work through `Fenix::Runtime::ExecuteMailboxItem`.
...
in `Core Matrix`, while `Fenix` remains an agent that provides prompt
construction, skills policy, and agent-owned tools.

The target end state is a complementary system:

- `Core Matrix` provides the loop kernel
- `Fenix` provides the agent intelligence
...
`Core Matrix` must not directly dispatch application work into an agent
...
- durable truth and publication for:
  ...
  - agent mailbox requests and reports
...
agent-specific prompt construction policy.

## Fenix Responsibilities

`Fenix` remains an agent, not the loop kernel.
...
- agent-specific tool exposure
- execution of agent-owned tools that depend on the Fenix runtime
  environment, including:
  - workspace and filesystem helpers
  - browser tooling
  - memory helpers
  - command and process helpers that remain agent-owned
...
agent-owned actions that `Core Matrix` routes to it through the mailbox-first
control plane.

## Why Provider Session Belongs In Core Matrix

Provider session and transport should move into `Core Matrix` because they are
kernel infrastructure rather than agent behavior.
...
- future agents should be able to reuse the same provider loop without
  re-implementing transport stacks

This keeps provider behavior unified even when the agent layer changes.
...
- they may cause `Fenix` to expose additional agent-owned tools
...
Without `Fenix`, `Core Matrix` has no agent-specific prompt shaping or
...
- ask the agent to prepare this round's cognition package
...
- agent-owned visible tools and JSON schemas
...
This keeps MCP support reusable across agents.
...
4. `Fenix` executes local prompt assembly and reports round messages plus
   agent-owned visible tools.
5. `Core Matrix` merges those agent-owned tools with any kernel-native and
...
8. If the model returns tool calls:
   ...
   - Fenix-owned tools are enqueued back to the agent through the
...
- `Fenix` owns prompt assembly, skills policy, and execution of agent-owned
  tools
- `Core Matrix` never directly dispatches application work into `Fenix` through
  synchronous RPC
- mailbox state remains the durable truth for cross-app agent work
...
- skills remain optional and agent-specific inside `Fenix`
