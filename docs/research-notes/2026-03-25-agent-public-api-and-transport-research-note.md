# Agent Public API And Transport Research Note
...
`agent <-> Core Matrix` communication. It should remain useful even if
...
- Canonical control semantics should be a durable mailbox model with leases and
  deadlines.
- `agents` should initiate outbound connections to `Core Matrix`; the
...
- durable connection credential issued after successful registration
...
- multiple non-Ruby agents need first-class client libraries
