...
The current `SubagentSession` model still persists three partially overlapping
signals:

- `close_state` from `ClosableRuntimeResource`
- `lifecycle_state` on `SubagentSession` itself
...
- Keep external and machine-facing payload shape stable where cheap:
  `SubagentSessions::Wait` and `SubagentSessions::ListForConversation` may keep
...
- Do not introduce a second compatibility wrapper or a third state object.
  The canonical owner should live directly on `SubagentSession`.
...
- Remove direct writes to `SubagentSession#lifecycle_state` before removing the
  column.
...
- `core_matrix/app/services/agent_control/create_resource_close_request.rb` and
  ...
  write `SubagentSession#lifecycle_state`
...
- `SubagentSession` exposes one canonical mapping from `close_state` to
...
test "derives lifecycle projection from close_state" do
  session = build_subagent_session(close_state: "acknowledged")
...
git commit -m "refactor: consolidate subagent session state ownership"
```
...
- Modify: `core_matrix/app/services/subagent_sessions/request_close.rb`
- Test: `core_matrix/test/services/subagent_sessions/request_close_test.rb`
...
- terminal close reports for `SubagentSession` only settle:
  - `close_state`
...
```ruby
test "resource_closed terminalizes a subagent session through close_state" do
  result = AgentControl::Report.call(...)

  assert_equal "accepted", result.code
  assert_equal "closed", subagent_session.reload.close_state
  assert_equal "interrupted", subagent_session.last_known_status
...
- `AgentControl::CreateResourceCloseRequest`
  - stop writing `SubagentSession#lifecycle_state`
...
- Modify: `core_matrix/app/services/subagent_sessions/list_for_conversation.rb`
...
- Test: `core_matrix/test/services/subagent_sessions/wait_test.rb`
...
- blocker counts use canonical subagent scopes instead of persisted lifecycle
  duplication
- quiescence rejects open or close-pending subagent sessions using the model's
...
- `SubagentSessions::Wait` and `SubagentSessions::ListForConversation` still
...
`SubagentSession.lifecycle_state`.
...
git commit -m "refactor: unify subagent session read-side close semantics"
```
...
- `close_state` is the durable close owner for `SubagentSession`
...
rg -n "SubagentSession.*lifecycle_state|lifecycle_close_requested\\?|lifecycle_closed\\?|lifecycle_open\\?" \
  core_matrix/app \
  core_matrix/test \
...
- direct writes and enum-style lifecycle predicates for `SubagentSession`
  should be gone
...
```bash
git add core_matrix/docs/behavior/subagent-sessions-and-execution-leases.md \
...
- quiescence, blocker, wait, interrupt, and close-report paths no longer rely
  on a separate persisted `SubagentSession.lifecycle_state`.
