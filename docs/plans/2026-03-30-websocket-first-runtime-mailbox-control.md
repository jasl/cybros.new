...
**Goal:** Productize external and bundled runtime execution around mailbox-first realtime control, attached command tools, and long-lived process control without relying on `/runtime/executions`.

**Architecture:** Keep `Core Matrix` as the only orchestration truth and move `Fenix` to a true mailbox worker model. Short-lived commands become attached `ToolInvocation` sub-execution via `exec_command` and `write_stdin`; long-lived services remain `ProcessRun`-owned environment resources with separate close/output handling. Remove `/runtime/executions` as a product path and update manual proof scripts to run only through the real mailbox control plane.
...
### Task 6: Replace manual validation with mailbox-first proof scripts

**Files:**
- Modify: `/Users/jasl/Workspaces/Ruby/cybros/core_matrix/script/manual/phase2_acceptance_support.rb`
...
- Create/Modify: `/Users/jasl/Workspaces/Ruby/cybros/core_matrix/script/manual/phase2_*`
...
old execution surface.

**Step 3: Write minimal implementation**

- Update manual support to use only realtime/poll/report.
