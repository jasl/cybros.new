...
making the codebase ready for a future physical split between agent-program and
executor-program runtimes.
...
- `program_plane` for agent-program requests such as `prepare_round`,
  `execute_program_tool`, and supervision controls
- `executor_plane` for executor-program tools such as `exec_command`,
...
- `Agent` owns agent-program behavior, prompts, skills, memory, and
  supervision/program request handling
- `Executor` owns executor-program tools, resource lifecycles, and
...
- `Fenix::Shared` must not depend on `Fenix::Agent` or `Fenix::Executor`
- `Fenix::Agent` may depend on `Fenix::Shared`
- `Fenix::Executor` may depend on `Fenix::Shared`
- `Fenix::Agent` and `Fenix::Executor` must not depend directly on each other
...
The following capabilities belong in `Fenix::Executor`:
...
- `Agent` contributes program-plane contract and agent tool catalog
- `Executor` contributes executor-plane tool catalog and resource capabilities
...
- `agent_program_request` routes to `Agent`
...
- `Fenix::Agent`, `Fenix::Executor`, and `Fenix::Shared` are the active runtime
  namespaces
