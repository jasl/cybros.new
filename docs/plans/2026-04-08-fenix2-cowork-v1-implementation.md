...
> This file keeps its original `fenix2` filename for continuity, but the active
> cowork app now lives at `agents/fenix`. The previous product lives at
> `agents/fenix.old` and is reference-only during implementation.
