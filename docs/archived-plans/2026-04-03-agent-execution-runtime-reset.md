# Agent And Execution Runtime Reset Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the current conversation-bound deployment and execution-environment model with a turn-bound runtime model built around `Agent`, `AgentSnapshot`, optional `ExecutionRuntime`, and single-active session authentication.

**Architecture:** Reset the domain in one destructive migration pass. `Conversation` binds only to `Agent`. `Turn` freezes `AgentSnapshot` and optional `ExecutionRuntime`. Live authentication and control-plane ownership move to `AgentConnection` and `ExecutionRuntimeConnection`. Capability assembly, attachment delivery, recovery, and bundled `Fenix` bootstrap are rewritten around the new model. Obsolete services, docs, and tests are deleted instead of shimmed.
...
- Approved design:
  - `docs/finished-plans/2026-04-03-agent-execution-runtime-reset-design.md`
...
**Files:**
- Create: `docs/finished-plans/2026-04-03-agent-execution-runtime-reset-design.md`
- Create: `docs/finished-plans/2026-04-03-agent-execution-runtime-reset.md`
...
test -f docs/finished-plans/2026-04-03-agent-execution-runtime-reset-design.md
test -f docs/finished-plans/2026-04-03-agent-execution-runtime-reset.md
```
...
- Test: `core_matrix/test/models/agent_test.rb`
- Test: `core_matrix/test/models/user_agent_binding_test.rb`
- Test: `core_matrix/test/models/agent_snapshot_test.rb`
- Test: `core_matrix/test/models/execution_runtime_test.rb`
- Test: `core_matrix/test/models/agent_connection_test.rb`
- Test: `core_matrix/test/models/execution_runtime_connection_test.rb`
...
- conversation binds only to agent
- turn requires agent agent snapshot
...
```bash
cd core_matrix
bin/rails test test/models/agent_test.rb test/models/user_agent_binding_test.rb test/models/agent_snapshot_test.rb test/models/execution_runtime_test.rb test/models/agent_connection_test.rb test/models/execution_runtime_connection_test.rb test/models/conversation_test.rb test/models/turn_test.rb test/models/process_run_test.rb
```
...
- default test contexts produce `agent`, `agent_snapshot`,
...
- turn freezes the active agent snapshot
...
- freeze agent snapshot from session
...
### Task 5: Rewrite agent-plane and execution-plane sessions, routing, and controllers
...
- import targets agent, not deployment
