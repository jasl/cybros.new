...
- connection credential rotation issues a fresh secret and invalidates the previous credential atomically
- connection credential revocation makes the current credential unusable before any later re-registration
...
- connection credential rotation must issue a fresh secret, invalidate the previous credential atomically, and create an audit row
- connection credential revocation must make the current credential unusable and create an audit row before any later re-registration
...
- actual landed scope:
  ...
  - updated the manual validation checklist with a reproducible credential
    lifecycle runner
  - added
    `core_matrix/docs/behavior/agent-connection-credential-lifecycle-controls.md`
