...
- business execution behavior should live in pluggable agents
...
`agent`, `agent profile`, and `runtime contract`.
...
2. The agent returns intent and reports, not direct truth mutation.
...
agent is plugged into the kernel.
...
```json
{
  ...
  "capability_projection": {
    ...
    "parent_subagent_connection_id": null,
    "subagent_depth": 0,
...
- agent-owned execution outside provider turn loops
...
- `tool_surface` is the visible agent-owned tool catalog for this round
...
- the agent may emit them in progress or terminal reports
...
- the agent consumes that `tool_surface`
...
Most importantly, the kernel becomes easier to extend to new agents
