class CreateAgentPrograms < ActiveRecord::Migration[8.2]
  def change
    create_table :agent_programs do |t|
    ...
    add_index :agent_programs, [:installation_id, :key], unique: true
    add_index :agent_programs, [:installation_id, :visibility]
    add_index :agent_programs, :public_id, unique: true
  end
end
