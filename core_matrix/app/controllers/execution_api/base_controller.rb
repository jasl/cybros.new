module ExecutionAPI
  class BaseController < ProgramAPI::BaseController
    ...
    before_action :authenticate_execution_session!

    private

    attr_reader :current_execution_session, :current_execution_runtime

    def authenticate_execution_session!
      @current_execution_session = authenticate_with_http_token do |token, _options|
        ExecutionSession.find_by_plaintext_session_credential(token)
      end
      @current_execution_runtime = @current_execution_session&.execution_runtime
      return if @current_execution_session.present?
