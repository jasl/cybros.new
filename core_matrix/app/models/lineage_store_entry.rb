class LineageStoreEntry < ApplicationRecord
  ENTRY_KINDS = %w[set tombstone].freeze

  belongs_to :lineage_store_snapshot
  belongs_to :lineage_store_value, optional: true

  validates :key, presence: true, uniqueness: { scope: :lineage_store_snapshot_id }
  ...
  def entry_value_rules
    if entry_kind == "set"
      errors.add(:lineage_store_value, "must exist for set entries") if lineage_store_value.blank?
    ...
    errors.add(:lineage_store_value, "must be blank for tombstone entries") if lineage_store_value.present?
