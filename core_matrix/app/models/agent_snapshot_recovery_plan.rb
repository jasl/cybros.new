class AgentSnapshotRecoveryPlan
  ...
  def normalize_recovery_target(recovery_target)
    ...
    return recovery_target if recovery_target.is_a?(AgentSnapshotRecoveryTarget)

    raise ArgumentError, "recovery target must be an AgentSnapshotRecoveryTarget"
  end
