class SubagentConnection < ApplicationRecord
  include HasPublicId
  ...
  has_many :child_subagent_connections,
    class_name: "SubagentConnection",
    foreign_key: :parent_subagent_connection_id,
    dependent: :restrict_with_exception,
    inverse_of: :parent_subagent_connection
  ...
  def derived_close_status
    DERIVED_CLOSE_STATUS_BY_CLOSE_STATE.fetch(close_state) do
      raise ArgumentError, "unsupported subagent connection close state #{close_state.inspect}"
    end
  end
  ...
  def parent_subagent_connection_installation_match
    return if parent_subagent_connection.blank? || parent_subagent_connection.installation_id == installation_id

    errors.add(:parent_subagent_connection, "must belong to the same installation")
  end
  ...
  def depth_consistency
    if parent_subagent_connection.blank?
