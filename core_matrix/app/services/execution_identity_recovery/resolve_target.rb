module ExecutionIdentityRecovery
  class ResolveTarget
    ...
    def initialize(
      ...
      same_logical_agent_as: turn.agent_definition_version,
    ...
    def call
      ...
      ExecutionIdentityRecoveryTarget.new(
        agent_definition_version: @agent_definition_version,
    ...
    def validate_same_installation!
      return if @agent_definition_version.installation_id == @conversation.installation_id

      raise_invalid!(:agent_definition_version, "must belong to the same installation", reason: "installation_drift")
    end
    ...
    def validate_same_environment!
      ...
      candidate_runtime_id = resolved_agent_definition_version.agent.default_execution_runtime_id
      return if candidate_runtime_id == expected_runtime_id

      raise_invalid!(
        :agent_definition_version,
        "must preserve the frozen execution runtime",
        reason: "execution_runtime_drift"
      )
    end

    def validate_same_logical_agent!
      return if @same_logical_agent_as.same_logical_agent?(resolved_agent_definition_version)
    ...
    def validate_capability_contract!
      return if resolved_agent_definition_version.preserves_capability_contract?(@capability_contract_turn)
    ...
    def probe_turn
      @probe_turn ||= @turn.dup.tap do |turn|
        ...
        turn.agent_definition_version = resolved_agent_definition_version
        turn.execution_runtime = @turn.execution_runtime
        turn.execution_runtime_version = @turn.execution_runtime_version
        turn.pinned_agent_definition_fingerprint = resolved_agent_definition_version.definition_fingerprint
        turn.agent_config_version = @turn.agent_config_version
        turn.agent_config_content_fingerprint = @turn.agent_config_content_fingerprint
