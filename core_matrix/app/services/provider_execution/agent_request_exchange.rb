  ...
  class AgentRequestExchange
    ...
    class PendingResponse < StandardError
      ...
      def initialize(mailbox_item_public_id:, logical_work_id:, request_kind:)
        ...
        super("agent request #{request_kind} is pending for mailbox item #{mailbox_item_public_id}")
      end
    end
    class RequestFailed < ExchangeError
      ...
      def initialize(error_payload:, details: {}, retryable: false)
        ...
        super(
          code: @error_payload.fetch("code", "agent_request_failed"),
          message: @error_payload.fetch("message", "agent request failed"),
          details: details,
          retryable:
        )
      end
    ...
    def execute_tool(payload:)
      perform_request!(
        ...
        payload:,
        logical_work_id: "tool-call:#{payload.fetch("task").fetch("workflow_node_id")}:#{payload.fetch("tool_call").fetch("call_id")}",
        timeout: execute_tool_timeout_for(payload),
    ...
    PENDING_METADATA_KEY = Workflows::BlockNodeForAgentRequest::METADATA_KEY
    ...
    def perform_request!(request_kind:, payload:, logical_work_id:, timeout:, allow_failure_response: false)
      ...
      raise ProtocolError.new(code: "missing_workflow_node_id", message: "agent requests must target a workflow node") if workflow_node.blank?
      ...
      Workflows::BlockNodeForAgentRequest.call(
        workflow_node: workflow_node,
        mailbox_item: mailbox_item,
        request_kind: request_kind,
        logical_work_id: logical_work_id,
    ...
    def terminal_response_for(workflow_node:, mailbox_item:, receipt:, request_kind:, allow_failure_response:)
      ...
      publish_wait_event!(
        mailbox_item: mailbox_item,
        request_kind: request_kind,
        success: receipt.payload.fetch("method_id") == "agent_completed",
        started_at: mailbox_item.created_at || mailbox_item.available_at || Time.current,
        finished_at: receipt.created_at || Time.current
      )
      report_payload = receipt.payload.deep_stringify_keys

      return report_payload.fetch("response_payload") if report_payload.fetch("method_id") == "agent_completed"
    ...
    def pending_mailbox_item_for!(workflow_node, pending_state)
      ...
      raise ProtocolError.new(
        ...
        message: "pending agent request is missing its mailbox item",
    ...
    def execute_tool_timeout_for(payload)
      requested_timeout_seconds = payload.dig("tool_call", "arguments", "timeout_seconds").to_f
      return @execute_tool_timeout if requested_timeout_seconds <= 0

      [@execute_tool_timeout.to_f, requested_timeout_seconds + @tool_timeout_buffer.to_f].max.seconds
    end
