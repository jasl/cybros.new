module ProviderExecution
  class PromptCompactionStrategy
    ...
    def initialize(messages:, hard_input_token_limit:, recommended_compaction_threshold:, selected_input_message_id:, tokenizer_hint: nil)
    ...
    def call
      ...
      if after_estimate.fetch("estimated_tokens") > @hard_input_token_limit
        minimal_messages = minimal_message_sequence
        compacted_messages = minimal_messages
        after_estimate = estimate(compacted_messages)
      end
    ...
    def estimate(messages)
      ...
      ).slice("estimated_tokens", "strategy", "diagnostics")
    end
