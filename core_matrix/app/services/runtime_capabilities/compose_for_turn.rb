  ...
  class ComposeForTurn
    ...
    def current_profile_key
      @current_profile_key ||= begin
        @turn.conversation.subagent_session&.profile_key ||
    ...
    def effective_subagent_policy
      contract.default_config_snapshot.fetch("subagents", {}).deep_merge(
        @turn.conversation.override_payload.fetch("subagents", {})
      )
    end
    ...
    def hide_subagent_spawn?
      ...
      return false unless @turn.conversation.subagent_session.present?
      return true if nested_spawning_disabled?

      max_depth = effective_subagent_policy["max_depth"]
      max_depth.present? && @turn.conversation.subagent_session.depth >= max_depth.to_i
    end
