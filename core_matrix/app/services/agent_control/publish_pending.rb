module AgentControl
  class PublishPending
    ...
    def initialize(mailbox_item: nil, agent_definition_version: nil, agent_connection: nil, execution_runtime_connection: nil, resolved_delivery_endpoint: nil, occurred_at: Time.current)
    ...
    def call
      return publish_for_agent_definition_version! if @agent_definition_version.present?
      ...
      target_agent_definition_version = connected_target_for(delivery_endpoint)
      ...
      leased_item = LeaseMailboxItem.call(
        mailbox_item: @mailbox_item,
        agent_definition_version: target_agent_definition_version,
      ...
      broadcast(mailbox_item: leased_item, delivery_endpoint: target_agent_definition_version)
      leased_item
    end

    private

    def publish_for_agent_definition_version!
      mailbox_items = Poll.call(agent_definition_version: @agent_definition_version, agent_connection: @agent_connection, limit: Poll::DEFAULT_LIMIT, occurred_at: @occurred_at)
      ...
      mailbox_items.zip(serialized_items).each do |mailbox_item, serialized_item|
        broadcast(mailbox_item:, delivery_endpoint: @agent_definition_version, serialized_item:)
      end
    end
    ...
    def connected_target_for(delivery_endpoint)
      ...
      when AgentConnection
        delivery_endpoint.agent_definition_version
    ...
    def publish_mailbox_lease_event!(mailbox_item:, target_agent_definition_version:, delivery_endpoint:, prior_delivery_no:)
      return unless mailbox_item.delivery_no > prior_delivery_no.to_i

      AgentControl::PublishMailboxLeaseEvent.call(
        mailbox_item: mailbox_item,
        agent_public_id: target_agent_definition_version.is_a?(AgentDefinitionVersion) ? target_agent_definition_version.agent.public_id : nil,
