#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CORE_MATRIX_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
REPO_ROOT="$(cd "${CORE_MATRIX_ROOT}/.." && pwd)"
ARTIFACT_STAMP="${CAPSTONE_ARTIFACT_STAMP:-2026-04-03-core-matrix-loop-fenix-2048-final}"
ARTIFACT_DIR="${REPO_ROOT}/docs/checklists/artifacts/${ARTIFACT_STAMP}"
BOOTSTRAP_STATE_PATH="${CAPSTONE_BOOTSTRAP_STATE_PATH:-${ARTIFACT_DIR}/capstone-runtime-bootstrap.json}"
RUNTIME_WORKER_BOOT_PATH="${CAPSTONE_RUNTIME_WORKER_BOOT_PATH:-${ARTIFACT_DIR}/docker-runtime-worker.json}"

export FENIX_RUNTIME_MODE="${FENIX_RUNTIME_MODE:-docker}"
export CAPSTONE_BOOTSTRAP_STATE_PATH="${BOOTSTRAP_STATE_PATH}"
export CAPSTONE_RUNTIME_WORKER_BOOT_PATH="${RUNTIME_WORKER_BOOT_PATH}"

bash "${SCRIPT_DIR}/fresh_start_stack.sh"

cd "${CORE_MATRIX_ROOT}"
CAPSTONE_PHASE=bootstrap bin/rails runner script/manual/acceptance/fenix_capstone_app_api_roundtrip_validation.rb

machine_credential="$(
  ruby -rjson -e 'state = JSON.parse(File.read(ARGV[0])); puts state.fetch("machine_credential")' \
    "${BOOTSTRAP_STATE_PATH}"
)"
execution_machine_credential="$(
  ruby -rjson -e 'state = JSON.parse(File.read(ARGV[0])); puts state.fetch("execution_machine_credential")' \
    "${BOOTSTRAP_STATE_PATH}"
)"

FENIX_MACHINE_CREDENTIAL="${machine_credential}" \
FENIX_EXECUTION_MACHINE_CREDENTIAL="${execution_machine_credential}" \
CAPSTONE_RUNTIME_WORKER_BOOT_PATH="${RUNTIME_WORKER_BOOT_PATH}" \
bash "${SCRIPT_DIR}/activate_fenix_docker_runtime.sh"

CAPSTONE_PHASE=execute bin/rails runner script/manual/acceptance/fenix_capstone_app_api_roundtrip_validation.rb
