...
- A binding links one user to one logical `AgentInstallation`.
...
- Bindings stay inside one installation and inherit that boundary from both the
  user and the agent installation.
- Personal agent installations may only be bound by their owner user.
...
- Rejects enabling another user's personal agent installation.
