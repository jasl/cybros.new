...
class UserBindingWorkspaceFlowTest < ActionDispatch::IntegrationTest
  test "enables a shared agent without duplicate bindings and keeps workspaces private" do
    ...
    first_enable = UserAgentBindings::Enable.call(user: first_user, agent_installation: shared_agent)
    duplicate_enable = UserAgentBindings::Enable.call(user: first_user, agent_installation: shared_agent)
    second_enable = UserAgentBindings::Enable.call(user: second_user, agent_installation: shared_agent)

    assert_equal first_enable.binding, duplicate_enable.binding
    assert_equal first_enable.workspace, duplicate_enable.workspace
    assert_equal 2, UserAgentBinding.where(agent_installation: shared_agent).count
    assert_equal 2, Workspace.where(user_agent_binding: UserAgentBinding.where(agent_installation: shared_agent), is_default: true).count
