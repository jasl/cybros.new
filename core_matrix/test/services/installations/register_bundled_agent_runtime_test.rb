...
class Installations::RegisterBundledAgentRuntimeTest < ActiveSupport::TestCase
  test "reconciles bundled registry rows idempotently before any binding exists" do
    ...
    assert_equal true, first.agent_definition_version.default_canonical_config.dig("features", "title_bootstrap", "enabled")
    assert_equal "runtime_first", first.agent_definition_version.default_canonical_config.dig("features", "title_bootstrap", "mode")
    assert_equal true, first.agent_definition_version.default_canonical_config.dig("features", "prompt_compaction", "enabled")
    assert_equal "runtime_first", first.agent_definition_version.default_canonical_config.dig("features", "prompt_compaction", "mode")
