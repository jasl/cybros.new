...
class AgentSnapshots::AutoResumeWorkflowsTest < ActiveSupport::TestCase
  test "automatically resumes waiting workflows when runtime identity did not drift" do
    ...
    assert_equal context[:agent_snapshot].public_id, context[:workflow_run].reload.blocking_resource_id
    ...
    resumed = AgentSnapshots::AutoResumeWorkflows.call(agent_snapshot: context[:agent_snapshot])
  ...
  test "compatible rotated agent_snapshot auto resumes waiting workflows and rewrites turn pinning" do
    context = build_waiting_recovery_context!
    ...
    resumed = AgentSnapshots::AutoResumeWorkflows.call(agent_snapshot: replacement)

    assert_equal [context[:workflow_run].id], resumed.map(&:id)
    assert_equal context[:agent], context[:conversation].reload.agent
    turn = context[:turn].reload
    assert_equal replacement, turn.agent_snapshot
    assert_equal replacement.fingerprint, turn.pinned_agent_snapshot_fingerprint
    assert_equal replacement.public_id, turn.execution_snapshot.identity["agent_snapshot_id"]
    assert context[:workflow_run].reload.ready?
  end

  test "compatible rotated agent_snapshot auto resumes through the canonical rebinding owner" do
    context = build_waiting_recovery_context!
    replacement = create_compatible_replacement_agent_snapshot!(
      installation: context[:installation],
      agent: context[:agent],
  ...
  test "rechecks retained conversation state before auto resuming a workflow" do
    ...
    service = AgentSnapshots::AutoResumeWorkflows.new(agent_snapshot: context[:agent_snapshot])
