...
class AgentSnapshots::BootstrapTest < ActiveSupport::TestCase
  test "creates a system owned bootstrap workflow and audit row" do
    context = prepare_workflow_execution_setup!(create_workspace_context!)

    result = AgentSnapshots::Bootstrap.call(
    ...
    assert_equal "AgentSnapshot", result.turn.source_ref_type
    assert_equal context[:agent_snapshot].public_id, result.turn.source_ref_id
    assert_equal context[:agent_snapshot], result.turn.agent_snapshot
    assert_equal context[:agent_snapshot].fingerprint, result.turn.pinned_agent_snapshot_fingerprint

    workflow_run = result.workflow_run
    assert workflow_run.active?
    assert_equal result.turn, workflow_run.turn
    assert_equal ["agent_snapshot_bootstrap"], workflow_run.workflow_nodes.order(:ordinal).pluck(:node_key)
    ...
    audit_log = AuditLog.find_by!(action: "agent_snapshot.bootstrap_started")
    assert_equal context[:agent_snapshot], audit_log.subject
    assert_equal workflow_run.id, audit_log.metadata["workflow_run_id"]
    assert_equal result.turn.id, audit_log.metadata["turn_id"]
  end

  test "rejects workspaces outside the agent_snapshot installation without creating side effects" do
    context = prepare_workflow_execution_setup!(create_workspace_context!)
    ...
    counts_before = [
      ...
      Turn.count,
      WorkflowRun.count,
      AuditLog.where(action: "agent_snapshot.bootstrap_started").count,
    ]

    error = assert_raises(ArgumentError) do
      AgentSnapshots::Bootstrap.call(
        agent_snapshot: context[:agent_snapshot],
