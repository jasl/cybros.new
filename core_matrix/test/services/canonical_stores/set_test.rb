...
class CanonicalStores::SetTest < ActiveSupport::TestCase
  test "creates a new write snapshot and moves the conversation reference" do
    ...
    visible = CanonicalStores::GetQuery.call(reference_owner: context[:conversation], key: "tone")

    assert_equal "direct", visible.typed_value_payload["value"]
    assert_equal 1, context[:conversation].reload.canonical_store_reference.canonical_store_snapshot.depth
  end
  ...
  test "compacts before writing once the snapshot chain reaches depth 32" do
    ...
    assert_equal 32, context[:conversation].reload.canonical_store_reference.canonical_store_snapshot.depth

    assert_difference("CanonicalStoreSnapshot.count", +2) do
      CanonicalStores::Set.call(
        conversation: context[:conversation],
        key: "overflow",
