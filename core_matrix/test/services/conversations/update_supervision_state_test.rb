...
class Conversations::UpdateSupervisionStateTest < ActiveSupport::TestCase
  ...
  test "projects task rollups and active plan items into durable conversation supervision state" do
    ...
    assert_equal "renderer",
      state.conversation_supervision_state_detail.status_payload.fetch("current_turn_plan_summary").fetch("current_item_key")
  end
  ...
  test "subagent barrier summaries only include sessions attached to the blocking barrier" do
    ...
    barrier_session = SubagentConnection.create!(
      ...
      user: barrier_conversation.user,
      workspace: barrier_conversation.workspace,
      agent: barrier_conversation.agent,
      scope: "conversation",
      profile_key: "worker",
      depth: 0,
      observed_status: "running",
    ...
    SubagentConnection.create!(
      ...
      user: unrelated_conversation.user,
      workspace: unrelated_conversation.workspace,
      agent: unrelated_conversation.agent,
      scope: "conversation",
      profile_key: "worker",
      depth: 0,
      observed_status: "running",
  ...
  def build_supervision_with_turn_todo_plan_fixture!
    ...
    agent_task_run = create_agent_task_run!(
      ...
      started_at: 3.minutes.ago,
      supervision_state: "running",
      request_summary: "Stale prior request summary",
      current_focus_summary: "Stale prior focus summary",
      recent_progress_summary: "Finished reviewing the old models",
      next_step_hint: "Rewrite the migrations",
      last_progress_at: 1.minute.ago,
      supervision_payload: {}
