...
class Conversations::CreationSupportTest < ActiveSupport::TestCase
  ...
  test "skips child lineage reference creation when the parent has no lineage store reference" do
    ...
    assert_nil parent.reload.lineage_store_reference

    assert_no_difference("LineageStoreReference.count") do
      assert_nil harness.create_lineage_store_reference_for!(child, parent: parent)
    end
  end
