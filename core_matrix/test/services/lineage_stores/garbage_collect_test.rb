...
class LineageStores::GarbageCollectTest < ActiveSupport::TestCase
  test "preserves snapshots still reachable from a child conversation reference" do
    ...
    assert_no_difference("LineageStore.count") do
  ...
  test "deletes unreachable snapshots entries values and stores once the last reference is gone" do
    ...
    assert_equal 0, LineageStore.where(id: context[:lineage_store].id).count
    assert_equal 0, LineageStoreSnapshot.where(lineage_store: context[:lineage_store]).count
    assert_equal 0, LineageStoreEntry.count
    assert_equal 0, LineageStoreValue.count
  end

  test "reconciles unfinished delete close operations after removing the root lineage store blocker" do
    context = build_lineage_store_context!
