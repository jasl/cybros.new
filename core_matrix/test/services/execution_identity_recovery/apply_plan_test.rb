...
class ExecutionIdentityRecovery::ApplyPlanTest < ActiveSupport::TestCase
  test "applies a resume plan by restoring ready state" do
    context = build_waiting_recovery_context!
    plan = ExecutionIdentityRecoveryPlan.new(action: "resume")

    applied = ExecutionIdentityRecovery::ApplyPlan.call(
  ...
  test "does not apply a resume with rebind plan after the replacement agent definition version stops being schedulable" do
    context = build_waiting_recovery_context!
  ...
  test "applies a preplanned resume with rebind after the selected provider candidate becomes unavailable" do
    context = build_waiting_recovery_context!
    replacement = create_compatible_replacement_agent_definition_version!(
      installation: context[:installation],
      agent: context[:agent],
    ...
    assert_equal context[:agent], context[:conversation].reload.agent
    assert_equal replacement, context[:turn].reload.agent_definition_version
  end
  ...
  test "applies a resume with rebind plan through the canonical rebinding owner without re-resolving the target" do
    ...
    ExecutionIdentityRecovery::ResolveTarget.singleton_class.define_method(:call) do |*args, **kwargs|
      flunk("ApplyPlan must reuse the recovery target from the planner instead of re-resolving it")
    end
    ...
    assert_equal context[:agent], context[:conversation].reload.agent
    assert_equal replacement, context[:turn].reload.agent_definition_version
    assert_equal replacement.public_id, context[:turn].reload.execution_snapshot.identity["agent_definition_version_id"]
  ensure
