...
class AgentDefinitionVersions::UnavailablePauseStateTest < ActiveSupport::TestCase
  test "resume restores an unresolved paused blocker snapshot with the wait-state contract shape" do
    ...
    context[:workflow_run].update!(
      ...
      blocking_resource_id: context[:agent_definition_version].public_id
    )

    attributes = AgentDefinitionVersions::UnavailablePauseState.resume_attributes(
