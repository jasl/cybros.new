...
class AgentApiCapabilitiesTest < ActionDispatch::IntegrationTest
  test "capabilities refresh returns separate program and execution contract sections" do
    ...
    assert_equal registration[:execution_runtime].runtime_fingerprint, response_body["runtime_fingerprint"]
    ...
    assert_equal ["exec_command"], response_body.fetch("execution_plane").fetch("tool_catalog").map { |entry| entry.fetch("tool_name") }
  ...
  test "capabilities handshake refreshes execution runtime capabilities without changing the frozen program version" do
    registration = register_agent_runtime!(
      profile_catalog: default_profile_catalog,
      config_schema_snapshot: profile_aware_config_schema_snapshot,
      conversation_override_schema_snapshot: subagent_policy_override_schema_snapshot,
    ...
    assert_equal contract.effective_tool_catalog, response_body.fetch("effective_tool_catalog")
    assert_equal contract.execution_plane, response_body.fetch("execution_plane")
    assert_equal true, registration[:execution_runtime].reload.capability_payload.dig("attachment_access", "request_attachment")
  end
