...
class AgentApiRegistrationsTest < ActionDispatch::IntegrationTest
  test "registration exchanges an enrollment token for a session credential and program contract" do
    ...
    executor_program = ExecutorProgram.find_by_public_id!(response_body.fetch("executor_program_id"))
    ...
    assert_equal executor_program.public_id, response_body["executor_program_id"]
    assert_equal "fenix-host-a", response_body["executor_fingerprint"]
    assert_equal true, response_body.dig("executor_capability_payload", "attachment_access", "request_attachment")
    assert response_body["executor_machine_credential"].present?
    ...
    refute_includes response.body, %("#{executor_program.id}")
  end

  test "registration allows agent programs without an executor program" do
    installation = create_installation!
    actor = create_user!(installation: installation, role: "admin")
    agent_program = create_agent_program!(installation: installation)
    enrollment = AgentEnrollments::Issue.call(
    ...
    response_body = JSON.parse(response.body)
    assert_nil response_body["executor_program_id"]
    assert_nil response_body["executor_fingerprint"]
    assert_equal [], response_body.fetch("executor_tool_catalog")
    assert_equal({}, response_body.fetch("executor_capability_payload"))
  end
  ...
  test "registration defaults runtime kind and connection metadata from endpoint metadata" do
    ...
    post "/agent_api/registrations",
      params: {
        enrollment_token: enrollment.plaintext_token,
        executor_fingerprint: "fenix-host-b",
    ...
    assert_equal "local", executor_program.kind
    assert_equal({ "transport" => "http", "base_url" => "https://agents.example.test" }, executor_program.connection_metadata)
    assert_equal({}, response_body.fetch("executor_capability_payload"))
  end
  ...
  test "registration rejects malformed execution capability payloads with an unprocessable response" do
    ...
    assert_no_difference("AgentProgramVersion.count") do
      post "/agent_api/registrations",
        params: {
          enrollment_token: enrollment.plaintext_token,
          executor_fingerprint: "fenix-host-c",
  ...
  test "registration rejects malformed program contract hashes with an unprocessable response" do
    ...
    assert_no_difference("AgentProgramVersion.count") do
      post "/agent_api/registrations",
        params: {
          enrollment_token: enrollment.plaintext_token,
          executor_fingerprint: "fenix-host-d",
