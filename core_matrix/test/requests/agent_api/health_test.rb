...
class AgentApiHealthTest < ActionDispatch::IntegrationTest
  test "health endpoint returns session-backed runtime health" do
    ...
    assert_equal registration[:agent_snapshot].fingerprint, response_body["fingerprint"]
    ...
    assert_equal registration[:agent_snapshot].protocol_version, response_body["protocol_version"]
    assert_equal registration[:agent_snapshot].sdk_version, response_body["sdk_version"]
