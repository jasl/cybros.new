...
class ExecutorApiAttachmentsControllerTest < ActionDispatch::IntegrationTest
  test "request_attachment returns a signed handle for an attachment on the turn snapshot" do
    ...
    assert_equal context[:executor_program].public_id, body.fetch("executor_program_id")
  ...
  test "request_attachment rejects an executor session from another runtime" do
    context = build_agent_control_context!
    ...
    other_registration = register_agent_runtime!(
      ...
      executor_program: create_executor_program!(installation: context[:installation]),
    ...
    post "/executor_api/attachments/request",
      ...
      headers: executor_api_headers(other_registration.fetch(:executor_machine_credential)),
      as: :json
