...
class LineageStoreValueTest < ActiveSupport::TestCase
  test "computes payload bytesize and caps values at 2 MiB" do
    exact_payload = {
      "type" => "string",
      "value" => "a" * (2.megabytes - { "type" => "string", "value" => "" }.to_json.bytesize),
    }
    exact_value = LineageStoreValue.new(typed_value_payload: exact_payload)

    oversized_payload = {
      "type" => "string",
      "value" => "a" * (2.megabytes + 1),
    }
    oversized_value = LineageStoreValue.new(typed_value_payload: oversized_payload)
