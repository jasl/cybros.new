...
class ProductionDatabaseConfigTest < ActiveSupport::TestCase
  ...
  test "production sqlite databases can be rooted under an explicit nexus storage directory" do
    config = render_database_yml("NEXUS_STORAGE_ROOT" => "/tmp/nexus-slot/storage")
    production = config.fetch("production")

    assert_equal "/tmp/nexus-slot/storage/production.sqlite3", production.dig("primary", "database")
    assert_equal "/tmp/nexus-slot/storage/production_cache.sqlite3", production.dig("cache", "database")
    assert_equal "/tmp/nexus-slot/storage/production_queue.sqlite3", production.dig("queue", "database")
  end
