...
class SystemToolRegistryTest < ActiveSupport::TestCase
  ...
  test "registry entries expose executor and catalog metadata" do
    exec_command_entry = SystemToolRegistry.fetch!("exec_command")

    assert_equal ToolExecutors::ExecCommand, exec_command_entry.fetch(:executor)
  ...
  test "browser registry entries expose the browser executor slice" do
    browser_open_entry = SystemToolRegistry.fetch!("browser_open")

    assert_equal ToolExecutors::Browser, browser_open_entry.fetch(:executor)
  ...
  test "process registry entries expose the detached process executor slice" do
    process_exec_entry = SystemToolRegistry.fetch!("process_exec")
    process_proxy_info_entry = SystemToolRegistry.fetch!("process_proxy_info")
