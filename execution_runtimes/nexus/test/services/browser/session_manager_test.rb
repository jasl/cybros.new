...
class Browser::SessionManagerTest < ActiveSupport::TestCase
  ...
  test "open registers a session and close removes it" do
    ...
    opened = Browser::SessionManager.call(
      action: "open",
    ...
    assert Browser::SessionManager.lookup(browser_session_id: browser_session_id)

    closed = Browser::SessionManager.call(
      action: "close",
      browser_session_id: browser_session_id
    )

    assert_equal true, closed.fetch("closed")
    assert_nil Browser::SessionManager.lookup(browser_session_id: browser_session_id)
  ...
  test "navigate get_content and screenshot dispatch through the stored host" do
    ...
    navigated = Browser::SessionManager.call(
      action: "navigate",
      browser_session_id: browser_session_id,
      url: "https://example.com/docs"
    )
    content = Browser::SessionManager.call(
      action: "get_content",
      browser_session_id: browser_session_id
    )
    screenshot = Browser::SessionManager.call(
      action: "screenshot",
      browser_session_id: browser_session_id
    )
  ...
  test "list and info expose registered session metadata" do
    ...
    Browser::SessionManager.call(
      action: "navigate",
      browser_session_id: browser_session_id,
      url: "https://example.com/docs"
    )

    listed = Browser::SessionManager.call(action: "list")
    info = Browser::SessionManager.call(action: "info", browser_session_id: browser_session_id)
  ...
  test "list and info can be scoped to the owning execution" do
    ...
    owned = Browser::SessionManager.call(
      action: "open",
      url: "https://example.com",
      host_factory: host_factory,
    ...
    listed = Browser::SessionManager.call(action: "list", runtime_owner_id: "task-1")
    info = Browser::SessionManager.call(
      action: "info",
    ...
    assert_raises(Browser::SessionManager::ValidationError) do
      Browser::SessionManager.call(
        action: "info",
        browser_session_id: owned.fetch("browser_session_id"),
        runtime_owner_id: "task-2"
  ...
  test "playwright host wraps broken pipes as host errors" do
    host = Browser::SessionManager::PlaywrightHost.allocate
    ...
    error = assert_raises(Browser::SessionManager::HostError) do
  ...
  test "playwright host falls back to a discovered managed browser root when the configured path is empty" do
    Dir.mktmpdir("nexus-playwright-root") do |tmpdir|
      ...
      host = Browser::SessionManager::PlaywrightHost.new(
