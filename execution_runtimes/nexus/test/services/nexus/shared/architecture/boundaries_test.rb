...
class NexusSharedArchitectureBoundariesTest < ActiveSupport::TestCase
  test "shared implementation files do not depend on agent or executor namespaces" do
    shared_root = Rails.root.join("app/services/nexus/shared")
    ...
    offenders = shared_files.filter_map do |path|
      ...
      next unless contents.match?(/\bNexus::(Agent|Executor)\b/)
  ...
  test "agent implementation files do not depend on runtime or legacy namespaces" do
    agent_root = Rails.root.join("app/services/nexus/agent")
    ...
    offenders = agent_files.filter_map do |path|
      contents = File.read(path)
      next unless contents.match?(/\bNexus::Runtime::|\bNexus::(Prompts|Memory|Hooks|Skills)\b/)
  ...
  test "execution runtime implementation files do not depend on runtime or legacy executor namespaces" do
    execution_runtime_root = Rails.root.join("app/services/nexus/execution_runtime")
    ...
    offenders = execution_runtime_files.filter_map do |path|
      contents = File.read(path)
      next unless contents.match?(/\bNexus::Runtime::|\bNexus::(Processes|Browser|Hooks)\b/)
