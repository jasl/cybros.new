...
class Nexus::Application::BuildRoundInstructionsTest < ActiveSupport::TestCase
  test "builds a system prompt plus transcript without inferring durable state from transcript" do
    Dir.mktmpdir("nexus-workspace-") do |workspace_root|
      ...
      root.join("AGENTS.md").write("Keep changes scoped to agents/nexus.\n")
      ...
      result = Nexus::Application::BuildRoundInstructions.call(context: context)
      ...
      assert_includes result.fetch("messages").first.fetch("content"), "Keep changes scoped to agents/nexus."
    end
  end
end
