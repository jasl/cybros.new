...
class Nexus::Agent::Skills::RepositoryTest < ActiveSupport::TestCase
  test "scopes writable roots under the nexus home root by agent and user" do
    with_skill_fixture_roots do |roots|
      repository = Nexus::Agent::Skills::Repository.new(
        agent_id: "agent-1",
        user_id: "user-1",
  ...
  test "shares installed skills across repositories in the same scope and isolates a different program scope" do
    with_skill_fixture_roots do |roots|
      source_root = Dir.mktmpdir("nexus-third-party-skill-")
      write_skill(root: source_root, name: "portable-notes", description: "Capture notes.", body: "Capture notes safely.")

      repository_a1 = Nexus::Agent::Skills::Repository.new(
        agent_id: "agent-1",
        user_id: "user-1",
        home_root: roots.fetch(:home_root),
        system_root: roots.fetch(:system_root),
      ...
      repository_a2 = Nexus::Agent::Skills::Repository.new(
        agent_id: "agent-1",
        user_id: "user-1",
        home_root: roots.fetch(:home_root),
        system_root: roots.fetch(:system_root),
      ...
      assert_raises(Nexus::Agent::Skills::Repository::SkillNotFound) do
  ...
  test "rejects installs that collide with reserved system skill names" do
    with_skill_fixture_roots do |roots|
      ...
      assert_raises(Nexus::Agent::Skills::Repository::ReservedSkillNameError) do
  ...
  test "requires non-blank and path-safe agent and user scope ids" do
    with_skill_fixture_roots do |roots|
      error = assert_raises(ArgumentError) do
        Nexus::Agent::Skills::Repository.new(
          agent_id: "",
          user_id: "user-1",
          home_root: roots.fetch(:home_root),
          system_root: roots.fetch(:system_root),
  ...
  test "read_file rejects symlink targets even when metadata validation is bypassed" do
    with_skill_fixture_roots do |roots|
      ...
      assert_raises(Nexus::Agent::Skills::Repository::InvalidFileReference) do
