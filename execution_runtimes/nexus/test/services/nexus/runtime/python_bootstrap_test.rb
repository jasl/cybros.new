...
require "fileutils"
require "open3"
require_relative "../../../../lib/nexus/runtime/python_bootstrap"

class Nexus::Runtime::PythonBootstrapTest < ActiveSupport::TestCase
  test "default_versions_path falls back to installed nexus versions when repo-local matrix is unavailable" do
    Dir.mktmpdir("nexus-python-bootstrap-versions") do |tmpdir|
      ...
      klass = Nexus::Runtime::PythonBootstrap.singleton_class
      ...
      begin
        assert_equal installed, Nexus::Runtime::PythonBootstrap.send(:default_versions_path)
      ensure
  ...
  test "ensure_ready provisions a managed python runtime under nexus home root and prepends it to PATH" do
    Dir.mktmpdir("nexus-python-bootstrap") do |tmpdir|
      fake_bin = File.join(tmpdir, "bin")
      home_root = File.join(tmpdir, "nexus-home")
      ...
      root = Nexus::Runtime::PythonBootstrap.ensure_ready!(
        env: env,
      ...
      assert_equal root.to_s, env.fetch("NEXUS_PYTHON_ROOT")
  ...
  test "ensure_ready does not reprovision when the managed runtime already matches the requested version" do
    Dir.mktmpdir("nexus-python-bootstrap-existing") do |tmpdir|
      fake_bin = File.join(tmpdir, "bin")
      home_root = Pathname(tmpdir).join("nexus-home")
      ...
      Nexus::Runtime::PythonBootstrap.ensure_ready!(
        env: env,
        versions_path: Rails.root.join("../../images/nexus/versions.env")
      )
