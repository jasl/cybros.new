...
class Nexus::Runtime::MailboxExecutionJobTest < ActiveSupport::TestCase
  setup do
  ...
  test "perform does not require control plane configuration when report delivery is disabled" do
    ...
    result = with_dispatch_mode_stub(
      ...
      Nexus::Runtime::MailboxExecutionJob.perform_now(
  ...
  test "perform uses explicit control plane context when queued report delivery is enabled" do
    ...
    captured_client = nil
    original_execute = Nexus::Runtime::ExecuteMailboxItem.method(:call)
    Nexus::Runtime::ExecuteMailboxItem.singleton_class.define_method(:call) do |mailbox_item:, deliver_reports:, control_client:|
      captured_client = control_client
      { "status" => "ok", "mailbox_item_id" => mailbox_item.fetch("item_id"), "deliver_reports" => deliver_reports }
    end

    result = Nexus::Runtime::MailboxExecutionJob.perform_now(
      execution_assignment_mailbox_item,
    ...
    assert_equal "ok", result.fetch("status")
    assert_instance_of Nexus::Shared::ControlPlane::Client, captured_client
  ensure
    Nexus::Runtime::ExecuteMailboxItem.singleton_class.define_method(:call, original_execute) if original_execute
  end
