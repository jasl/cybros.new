...
class Nexus::Runtime::MailboxWorkerTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @original_control_plane_client =
      if Nexus::Shared::ControlPlane.instance_variable_defined?(:@client)
        Nexus::Shared::ControlPlane.instance_variable_get(:@client)
      else
        :__undefined__
      end
  end
  ...
  teardown do
    ...
    if @original_control_plane_client == :__undefined__
      Nexus::Shared::ControlPlane.remove_instance_variable(:@client) if Nexus::Shared::ControlPlane.instance_variable_defined?(:@client)
    else
      Nexus::Shared::ControlPlane.client = @original_control_plane_client
    end
  end
  ...
  test "agent task close requests emit close lifecycle reports" do
    ...
    result = Nexus::Runtime::MailboxWorker.call(
  ...
  test "process run close requests are delegated to the local process manager when a durable process is present" do
    ...
    Nexus::ExecutionRuntime::Processes::Manager.spawn!(
  ...
  test "non-inline executable mailbox items enqueue the runtime mailbox job" do
    Nexus::Shared::ControlPlane.client = build_control_client
    mailbox_item = execution_assignment_mailbox_item

    result = nil

    assert_enqueued_with(job: Nexus::Runtime::MailboxExecutionJob) do
      result = Nexus::Runtime::MailboxWorker.call(mailbox_item: mailbox_item, inline: false)
    end
  ...
  test "queued executable mailbox items are settled by the runtime mailbox job" do
    client = build_control_client
    Nexus::Shared::ControlPlane.client = client
    mailbox_item = execution_assignment_mailbox_item
  ...
  test "non-inline executable mailbox items ignore duplicate deliveries for the same mailbox item" do
    ...
    assert_enqueued_jobs 1 do
      Nexus::Runtime::MailboxWorker.call(mailbox_item: mailbox_item, inline: false)
      Nexus::Runtime::MailboxWorker.call(mailbox_item: mailbox_item.deep_dup, inline: false)
    end
  end
  ...
  test "queued executable mailbox items serialize control plane context for out-of-process execution" do
    client = Nexus::Shared::ControlPlane::Client.new(
      base_url: "https://core-matrix.example.test",
      execution_runtime_connection_credential: "executor-secret"
    )
    mailbox_item = execution_assignment_mailbox_item
    ...
    assert_equal "executor-secret", serialized_context.fetch("execution_runtime_connection_credential")
  end
