...
class Nexus::ExecutionRuntime::Browser::SessionManagerTest < ActiveSupport::TestCase
  ...
  setup do
    Nexus::ExecutionRuntime::Browser::SessionManager.reset!
  end
  ...
  test "open registers a session and close removes it" do
    ...
    opened = Nexus::ExecutionRuntime::Browser::SessionManager.call(
      action: "open",
    ...
    assert Nexus::ExecutionRuntime::Browser::SessionManager.lookup(browser_session_id: browser_session_id)

    closed = Nexus::ExecutionRuntime::Browser::SessionManager.call(
      action: "close",
      browser_session_id: browser_session_id
    )

    assert_equal true, closed.fetch("closed")
    assert_nil Nexus::ExecutionRuntime::Browser::SessionManager.lookup(browser_session_id: browser_session_id)
  ...
  test "navigate get_content and screenshot dispatch through the stored host" do
    ...
    navigated = Nexus::ExecutionRuntime::Browser::SessionManager.call(
      action: "navigate",
      browser_session_id: browser_session_id,
      url: "https://example.com/docs"
    )
    content = Nexus::ExecutionRuntime::Browser::SessionManager.call(
      action: "get_content",
      browser_session_id: browser_session_id
    )
    screenshot = Nexus::ExecutionRuntime::Browser::SessionManager.call(
      action: "screenshot",
      browser_session_id: browser_session_id
    )
  ...
  test "list and info expose registered session metadata" do
    ...
    Nexus::ExecutionRuntime::Browser::SessionManager.call(
      action: "navigate",
      browser_session_id: browser_session_id,
      url: "https://example.com/docs"
    )

    listed = Nexus::ExecutionRuntime::Browser::SessionManager.call(action: "list")
    info = Nexus::ExecutionRuntime::Browser::SessionManager.call(action: "info", browser_session_id: browser_session_id)
  ...
  test "list and info can be scoped to the owning execution" do
    ...
    owned = Nexus::ExecutionRuntime::Browser::SessionManager.call(
      action: "open",
      url: "https://example.com",
      host_factory: host_factory,
    ...
    listed = Nexus::ExecutionRuntime::Browser::SessionManager.call(action: "list", runtime_owner_id: "task-1")
    info = Nexus::ExecutionRuntime::Browser::SessionManager.call(
      action: "info",
    ...
    assert_raises(Nexus::ExecutionRuntime::Browser::SessionManager::ValidationError) do
      Nexus::ExecutionRuntime::Browser::SessionManager.call(
        action: "info",
        browser_session_id: owned.fetch("browser_session_id"),
        runtime_owner_id: "task-2"
  ...
  test "playwright host wraps broken pipes as host errors" do
    host = Nexus::ExecutionRuntime::Browser::SessionManager::PlaywrightHost.allocate
    ...
    error = assert_raises(Nexus::ExecutionRuntime::Browser::SessionManager::HostError) do
  ...
  test "playwright host falls back to a discovered managed browser root when the configured path is empty" do
    Dir.mktmpdir("nexus-playwright-root") do |tmpdir|
      ...
      host = Nexus::ExecutionRuntime::Browser::SessionManager::PlaywrightHost.new(
  ...
  test "browser host can resolve a globally installed playwright package" do
    Dir.mktmpdir("nexus-global-playwright") do |tmpdir|
  ...
  test "browser host does not hide broken local playwright packages behind the global fallback" do
    Dir.mktmpdir("nexus-global-playwright-broken-local") do |tmpdir|
      script_path = write_session_host_copy(tmpdir)
      fake_bin = File.join(tmpdir, "bin")
