...
class Nexus::ExecutionRuntime::SystemToolRegistryTest < ActiveSupport::TestCase
  test "supported tool names cover the command-run and browser executor slices" do
    assert_equal(
      ...
      Nexus::ExecutionRuntime::SystemToolRegistry.supported_tool_names.sort
    )
  end

  test "registry entries expose executor and catalog metadata" do
    exec_command_entry = Nexus::ExecutionRuntime::SystemToolRegistry.fetch!("exec_command")

    assert_equal Nexus::ExecutionRuntime::ToolExecutors::ExecCommand, exec_command_entry.fetch(:executor)
  ...
  test "browser registry entries expose the browser executor slice" do
    browser_open_entry = Nexus::ExecutionRuntime::SystemToolRegistry.fetch!("browser_open")

    assert_equal Nexus::ExecutionRuntime::ToolExecutors::Browser, browser_open_entry.fetch(:executor)
  ...
  test "process registry entries expose the detached process executor slice" do
    process_exec_entry = Nexus::ExecutionRuntime::SystemToolRegistry.fetch!("process_exec")
    process_proxy_info_entry = Nexus::ExecutionRuntime::SystemToolRegistry.fetch!("process_proxy_info")
