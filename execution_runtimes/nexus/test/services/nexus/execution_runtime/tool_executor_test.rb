...
class Nexus::ExecutionRuntime::ToolExecutorTest < ActiveSupport::TestCase
  teardown do
    Nexus::ExecutionRuntime::CommandRunRegistry.reset!
    Nexus::ExecutionRuntime::Processes::Manager.reset!
    Nexus::ExecutionRuntime::Processes::ProxyRegistry.reset!
  end
  ...
  test "exec_command applies the workspace env overlay without mutating global ENV" do
    Dir.mktmpdir("nexus-workspace-") do |workspace_root|
      ...
      FileUtils.mkdir_p(root.join(".nexus"))
      root.join(".nexus", "workspace.env").write("HELLO=workspace\n")
  ...
  test "exec_command rejects invalid workspace env overlays as validation errors" do
    Dir.mktmpdir("nexus-workspace-") do |workspace_root|
      root = Pathname.new(workspace_root)
      FileUtils.mkdir_p(root.join(".nexus"))
      root.join(".nexus", "workspace.env").write("PATH=/tmp/fake\n")
      executor = build_executor(
        allowed_tool_names: ["exec_command"],
        workspace_root: workspace_root
      )

      error = assert_raises(Nexus::ExecutionRuntime::ToolExecutors::ExecCommand::ValidationError) do
        executor.call(
          tool_call: {
      ...
      assert_equal "validation_error", Nexus::ExecutionRuntime::ToolExecutor.error_payload_for(error).fetch("code")
    end
  end
  ...
  test "raises when the tool is not visible for this execution" do
    ...
    error = assert_raises(Nexus::ExecutionRuntime::ToolExecutor::ToolNotAllowedError) do
      executor.call(
        tool_call: {
    ...
    assert_equal "tool_not_allowed", Nexus::ExecutionRuntime::ToolExecutor.error_payload_for(error).fetch("code")
  end
  ...
  test "process_exec rejects invalid workspace env overlays as validation errors" do
    Dir.mktmpdir("nexus-workspace-") do |workspace_root|
      ...
      error = assert_raises(Nexus::ExecutionRuntime::ToolExecutors::Process::ValidationError) do
        executor.call(
          tool_call: {
  ...
  test "maps browser validation failures to semantic error payloads" do
    ...
    error = assert_raises(Nexus::ExecutionRuntime::Browser::SessionManager::ValidationError) do
      with_session_manager_stub(->(**) { raise Nexus::ExecutionRuntime::Browser::SessionManager::ValidationError, "unknown browser session" }) do
        executor.call(
          tool_call: {
  ...
  def with_session_manager_stub(callable)
    session_manager_class = Nexus::ExecutionRuntime::Browser::SessionManager
  ...
  def with_process_runtime_stubs(launcher:, manager_list:, manager_output_snapshot:, proxy_lookup:)
    execution_runtime_module = Nexus::ExecutionRuntime
  ...
  def build_executor(allowed_tool_names:, workspace_root: Dir.pwd)
    Nexus::ExecutionRuntime::ToolExecutor.new(
      context: {
