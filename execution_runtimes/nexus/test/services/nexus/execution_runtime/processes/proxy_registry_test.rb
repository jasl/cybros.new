...
class NexusProcessesProxyRegistryTest < ActiveSupport::TestCase
  teardown do
    Nexus::ExecutionRuntime::Processes::ProxyRegistry.reset_default!
  end
  ...
  test "register writes a stable caddy routes fragment and lookup returns the route" do
    Dir.mktmpdir("nexus-proxy-registry-") do |tmpdir|
      ...
      entry = Nexus::ExecutionRuntime::Processes::ProxyRegistry.new(routes_path: routes_path).register(
        process_run_id: "process-run-1",
        target_port: 4100
      )
      second_entry = Nexus::ExecutionRuntime::Processes::ProxyRegistry.new(routes_path: routes_path).register(
      ...
      assert_equal entry, Nexus::ExecutionRuntime::Processes::ProxyRegistry.new(routes_path: routes_path).lookup(process_run_id: "process-run-1")
      assert_equal second_entry, Nexus::ExecutionRuntime::Processes::ProxyRegistry.new(routes_path: routes_path).lookup(process_run_id: "process-run-2")
