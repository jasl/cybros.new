...
class ToolExecutorTest < ActiveSupport::TestCase
  ...
  test "exec_command rejects invalid workspace env overlays as validation errors" do
    Dir.mktmpdir("nexus-workspace-") do |workspace_root|
      ...
      error = assert_raises(ToolExecutors::ExecCommand::ValidationError) do
        executor.call(
          tool_call: {
      ...
      assert_equal "validation_error", ToolExecutor.error_payload_for(error).fetch("code")
    end
  end
  ...
  test "raises when the tool is not visible for this execution" do
    ...
    error = assert_raises(ToolExecutor::ToolNotAllowedError) do
      executor.call(
        tool_call: {
    ...
    assert_equal "tool_not_allowed", ToolExecutor.error_payload_for(error).fetch("code")
  end
  ...
  test "process_exec rejects invalid workspace env overlays as validation errors" do
    Dir.mktmpdir("nexus-workspace-") do |workspace_root|
      ...
      error = assert_raises(ToolExecutors::Process::ValidationError) do
        executor.call(
          tool_call: {
  ...
  test "maps browser validation failures to semantic error payloads" do
    ...
    error = assert_raises(Browser::SessionManager::ValidationError) do
      with_session_manager_stub(->(**) { raise Browser::SessionManager::ValidationError, "unknown browser session" }) do
        executor.call(
          tool_call: {
  ...
  def with_session_manager_stub(callable)
    session_manager_class = Browser::SessionManager
  ...
  def with_process_runtime_stubs(launcher:, manager_list:, manager_output_snapshot:, proxy_lookup:)
    ...
    processes_module.send(:remove_const, :Manager) if processes_module.const_defined?(:Manager, false)
    processes_module.send(:remove_const, :ProxyRegistry) if processes_module.const_defined?(:ProxyRegistry, false)
    processes_module.const_set(:Launcher, original_launcher) if original_launcher
    processes_module.const_set(:Manager, original_manager) if original_manager
    processes_module.const_set(:ProxyRegistry, original_proxy_registry) if original_proxy_registry
  end
