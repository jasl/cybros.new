module Nexus
  module Runtime
    class ControlWorker
      def initialize(
        limit: Nexus::Runtime::MailboxPump::DEFAULT_LIMIT,
        inline: false,
        control_client: nil,
        session_factory: nil,
        timeout_seconds: 5,
        mailbox_pump: Nexus::Runtime::MailboxPump,
        control_loop: Nexus::Runtime::ControlLoop,
        stop_condition: nil,
      ...
      def sweep_local_process_handles!
        Nexus::ExecutionRuntime::Processes::Manager.prune_terminated_handles!
      end
      ...
      def run_control_loop
        ...
        Rails.logger.error("[nexus.runtime.control_worker] #{error.class}: #{error.message}")
        Rails.logger.error(error.backtrace.join("\n")) if error.backtrace.present?

        Nexus::Runtime::ControlLoop::Result.new(
          transport: "error",
          realtime_result: Nexus::Runtime::RealtimeConnection::Result.new(
            status: "failed",
            processed_count: 0,
