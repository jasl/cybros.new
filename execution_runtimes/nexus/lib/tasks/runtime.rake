...
def pairing_manifest_payload
  Nexus::Runtime::Manifest::PairingManifest.call(
...
namespace :runtime do
  ...
  task mailbox_pump_once: :environment do
    limit = Integer(ENV.fetch("LIMIT", Nexus::Runtime::MailboxPump::DEFAULT_LIMIT))
    inline = ActiveModel::Type::Boolean.new.cast(ENV.fetch("INLINE", "false"))

    results = Nexus::Runtime::MailboxPump.call(limit: limit, inline: inline)
  ...
  task control_loop_once: :environment do
    ...
    result = Nexus::Runtime::ControlLoop.call(limit: limit, inline: inline, timeout_seconds: timeout_seconds)
  ...
  task control_loop_forever: :environment do
    ...
    worker = Nexus::Runtime::ControlWorker.new(
      limit: limit,
      inline: inline,
