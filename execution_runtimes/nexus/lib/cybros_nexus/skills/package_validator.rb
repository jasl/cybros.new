  ...
  module Skills
    class PackageValidator
      ...
      def initialize(skill_root:)
        @skill_root = File.expand_path(skill_root.to_s)
      end
      ...
      def validate_name!(name)
        raise InvalidSkillPackage, "skill name is required" if name.empty?
        raise InvalidSkillPackage, "skill name exceeds #{MAX_NAME_LENGTH} characters" if name.length > MAX_NAME_LENGTH
        raise InvalidSkillPackage, "skill name must match the directory name" unless name == File.basename(@skill_root)
      ...
      def validate_description!(description)
        raise InvalidSkillPackage, "skill description is required" if description.empty?
      ...
      def read_skill_md
        skill_md_path = File.join(@skill_root, "SKILL.md")
        raise InvalidSkillPackage, "#{@skill_root} is missing SKILL.md" unless File.file?(skill_md_path)
      ...
      def validate_file_tree!
        ...
        Find.find(@skill_root) do |path|
          ...
          relative_path = path.delete_prefix("#{@skill_root}/")
