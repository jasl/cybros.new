...
class RuntimeWorkerContractTest < ActiveSupport::TestCase
  test "runtime worker and puma agree on the standalone solid queue contract" do
    ...
    dev_proxy = Rails.root.join("bin/nexus-dev-proxy").read
    ...
    assert_match(/NEXUS_DEV_PROXY_ROUTES_FILE/, dev_proxy)
    assert_match(/respond "no dev route registered" 404/, caddy_config)
    assert_match(/NEXUS_DEV_PROXY_PORT=/, env_sample)
    assert_match(/NEXUS_DEV_PROXY_ROUTES_FILE=/, env_sample)
