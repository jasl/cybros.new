...
class Skills::PackageValidatorTest < ActiveSupport::TestCase
  test "accepts a valid skill package and returns normalized metadata" do
    with_skill_fixture_roots do |roots|
      ...
      metadata = Skills::PackageValidator.call(skill_root: skill_root)
  ...
  test "rejects a package when the frontmatter name does not match the directory name" do
    with_skill_fixture_roots do |roots|
      ...
      error = assert_raises(Skills::PackageValidator::InvalidSkillPackage) do
        Skills::PackageValidator.call(skill_root: skill_root)
      end
