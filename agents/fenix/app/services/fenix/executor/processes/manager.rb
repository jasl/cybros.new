    ...
    module Processes
      ...
      class << self
        ...
        def proxy_info(process_run_id:)
          proxy_entry = Fenix::Executor::Processes::ProxyRegistry.lookup(process_run_id:)
        ...
        def cleanup_entry(entry)
          ...
          Fenix::Executor::Processes::ProxyRegistry.unregister(process_run_id: entry.process_run_id)
          close_io(entry.stdin)
          close_io(entry.stdout)
          close_io(entry.stderr)
        end
