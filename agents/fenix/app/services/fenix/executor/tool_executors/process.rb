  ...
  module Executor
    module ToolExecutors
      module Process
        ...
        class << self
          def call(tool_call:, control_client:, current_runtime_owner_id:, process_run: nil, launcher: Fenix::Executor::Processes::Launcher, context: {}, **)
            Runtime.new(
              tool_call: tool_call,
        ...
        class Runtime
          ...
          def call
            ...
            when "process_list"
              { "entries" => Fenix::Executor::Processes::Manager.list(runtime_owner_id: @current_runtime_owner_id) }
            when "process_proxy_info"
              lookup_owned_process_run!

              Fenix::Executor::Processes::Manager.proxy_info(process_run_id: @tool_call.dig("arguments", "process_run_id")) || {
                "process_run_id" => @tool_call.dig("arguments", "process_run_id"),
                "proxy_path" => nil,
                "proxy_target_url" => nil,
              }
            when "process_read_output"
              snapshot = Fenix::Executor::Processes::Manager.output_snapshot(process_run_id: @tool_call.dig("arguments", "process_run_id"))
          ...
          def lookup_owned_process_run!
            process_run_id = @tool_call.dig("arguments", "process_run_id")
            entry = Fenix::Executor::Processes::Manager.lookup(process_run_id: process_run_id)
