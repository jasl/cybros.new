        ...
        class Runtime
          ...
          def call
            ...
            when "command_run_list"
              { "entries" => Fenix::Executor::CommandRunRegistry.list(runtime_owner_id: @current_runtime_owner_id) }
          ...
          def execute_write_stdin
            ...
            snapshot = Fenix::Executor::CommandRunRegistry.output_snapshot(command_run_id: command_run_id) || {
            ...
            Fenix::Executor::CommandRunRegistry.release(command_run_id: command_run_id) if wait_for_exit && command_run_id.present? && !command_run&.session_closed
          end
          ...
          def execute_command_run_wait
            ...
            Fenix::Executor::CommandRunRegistry.release(command_run_id: command_run_id) if command_run_id.present? && !command_run&.session_closed
          end
          ...
          def execute_command_run_terminate
            ...
            snapshot = Fenix::Executor::CommandRunRegistry.terminate(command_run_id: command_run_id)
            raise ValidationError, "unknown command run #{command_run_id}" if snapshot.blank?
          ...
          def start_command_run_session
            ...
            Fenix::Executor::CommandRunRegistry.register(
              command_run_id: command_run_id,
          ...
          def execute_one_shot_command(command_line:, timeout_seconds:, timeout_label:)
            ...
            if command_run_id.present?
              if preserve_snapshot
                Fenix::Executor::CommandRunRegistry.release(command_run_id: command_run_id)
              else
                Fenix::Executor::CommandRunRegistry.terminate(command_run_id: command_run_id)
              end
            end
          end
          ...
          def read_available_output(command_run:, timeout_seconds:, command_run_id: command_run.command_run_id)
            ...
            ready.first.each_with_object([]) do |io, output_chunks|
              ...
              sanitized = Fenix::Executor::CommandRunRegistry.sanitize_output_text(chunk)
              Fenix::Executor::CommandRunRegistry.append_output(
                command_run_id: command_run_id,
                stream: stream,
                text: sanitized
              )
          ...
          def lookup_owned_command_run!(command_run_id)
            command_run = Fenix::Executor::CommandRunRegistry.lookup(command_run_id: command_run_id)
