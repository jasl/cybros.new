  ...
  module Executor
    class ProgramToolExecutor
      ...
      def self.error_payload_for(error)
        ...
        when Fenix::Executor::ToolExecutors::ExecCommand::ValidationError
          {
            "classification" => "semantic",
        ...
        when Fenix::Executor::Browser::SessionManager::ValidationError
          {
            "classification" => "semantic",
            "code" => "validation_error",
            "message" => error.message,
        ...
        when Fenix::Executor::ToolExecutors::Process::ValidationError
          {
            "classification" => "semantic",
            "code" => "validation_error",
            "message" => error.message,
      ...
      def execute(tool_call:, tool_invocation: nil, command_run: nil, process_run: nil)
        ...
        tool_registry_entry = Fenix::Executor::SystemToolRegistry.fetch!(normalized_tool_call.fetch("tool_name"))
      ...
      def validate_tool_call!(tool_call)
        ...
        raise UnsupportedToolError, "unsupported tool #{tool_name}" unless Fenix::Executor::SystemToolRegistry.supported_tool_names.include?(tool_name)
