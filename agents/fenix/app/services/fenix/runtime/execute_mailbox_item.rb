  ...
  module Runtime
    class ExecuteMailboxItem
      ...
      def execute_agent_request
        case request_kind
        when "prepare_round"
          Fenix::Agent::Requests::PrepareRound.call(payload: mailbox_payload)
        when "execute_tool"
          Fenix::Agent::Requests::ExecuteTool.call(
            payload: mailbox_payload,
            supported_system_tool_names: Fenix::ExecutionRuntime::SystemToolRegistry.supported_tool_names,
            system_tool_executor: system_tool_executor
          )
        when "supervision_status_refresh", "supervision_guidance"
          Fenix::Agent::Requests::ExecuteConversationControlRequest.call(payload: mailbox_payload)
        else
      ...
      def execution_assignment_error_payload_for(error)
        ...
        when Fenix::Agent::Requests::ExecuteConversationControlRequest::InvalidRequestError
          {
            "classification" => "semantic",
