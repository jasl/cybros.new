module Requests
  class ExecuteConversationControlRequest
    ...
    def subagent_connection_id
      @payload["subagent_connection_id"].to_s.strip.presence
    end
    ...
    def validate_request!
      ...
      raise InvalidRequestError, "conversation_control.conversation_control_request_id is required" if conversation_control_request_id.blank?
      raise InvalidRequestError, "conversation_control.conversation_id is required" if conversation_id.blank?

      %w[request_kind target_kind target_public_id].each do |key|
      ...
      when "request_status_refresh"
        ...
        raise InvalidRequestError, "status refresh does not accept subagent_connection_id" if subagent_connection_id.present?
      when "send_guidance_to_active_agent"
        validate_session_target!
        raise InvalidRequestError, "supervision_guidance requires content" if content.blank?
        raise InvalidRequestError, "active-agent guidance does not accept subagent_connection_id" if subagent_connection_id.present?
    ...
    def validate_conversation_target!
      ...
      unless conversation_control_payload["target_public_id"] == conversation_id
        raise InvalidRequestError, "conversation control target_public_id must match conversation_id"
      end
    end

    def validate_subagent_target!
      unless conversation_control_payload["target_kind"] == "subagent_connection"
        raise InvalidRequestError, "subagent guidance target_kind must be subagent_connection"
      end
      raise InvalidRequestError, "subagent guidance requires subagent_connection_id" if subagent_connection_id.blank?
      unless conversation_control_payload["target_public_id"] == subagent_connection_id
        raise InvalidRequestError, "subagent guidance target_public_id must match subagent_connection_id"
      end
    end

    def conversation_control_request_id
      conversation_control_payload["conversation_control_request_id"].presence
    end

    def conversation_id
      conversation_control_payload["conversation_id"].presence
    end

    alias_method :validate_session_target!, :validate_conversation_target!
  end
end
