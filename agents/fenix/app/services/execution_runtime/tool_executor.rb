module ExecutionRuntime
  class ToolExecutor
    ...
    def self.error_payload_for(error)
      ...
      when ExecutionRuntime::ToolExecutors::ExecCommand::ValidationError
        {
          "classification" => "semantic",
      ...
      when ExecutionRuntime::Browser::SessionManager::ValidationError
        {
          "classification" => "semantic",
          "code" => "validation_error",
          "message" => error.message,
      ...
      when ExecutionRuntime::ToolExecutors::Process::ValidationError
        {
          "classification" => "semantic",
          "code" => "validation_error",
          "message" => error.message,
    ...
    def execute(tool_call:, tool_invocation: nil, command_run: nil, process_run: nil)
      ...
      tool_registry_entry = ExecutionRuntime::SystemToolRegistry.fetch!(normalized_tool_call.fetch("tool_name"))
    ...
    def validate_tool_call!(tool_call)
      ...
      raise UnsupportedToolError, "unsupported tool #{tool_name}" unless ExecutionRuntime::SystemToolRegistry.supported_tool_names.include?(tool_name)
