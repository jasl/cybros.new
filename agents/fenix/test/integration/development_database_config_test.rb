...
class DevelopmentDatabaseConfigTest < ActiveSupport::TestCase
  ...
  test "development sqlite databases can be rooted under an explicit fenix storage directory" do
    config = render_database_yml("FENIX_STORAGE_ROOT" => "/tmp/fenix-slot/storage")
    development = config.fetch("development")

    assert_equal "/tmp/fenix-slot/storage/development.sqlite3", development.dig("primary", "database")
    assert_equal "/tmp/fenix-slot/storage/development_queue.sqlite3", development.dig("queue", "database")
  end
