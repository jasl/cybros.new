...
class ExecutionRuntime::Browser::SessionManagerTest < ActiveSupport::TestCase
  ...
  setup do
    ExecutionRuntime::Browser::SessionManager.reset!
  end
  ...
  test "open registers a session and close removes it" do
    ...
    opened = ExecutionRuntime::Browser::SessionManager.call(
      action: "open",
    ...
    assert ExecutionRuntime::Browser::SessionManager.lookup(browser_session_id: browser_session_id)

    closed = ExecutionRuntime::Browser::SessionManager.call(
      action: "close",
      browser_session_id: browser_session_id
    )

    assert_equal true, closed.fetch("closed")
    assert_nil ExecutionRuntime::Browser::SessionManager.lookup(browser_session_id: browser_session_id)
  ...
  test "navigate get_content and screenshot dispatch through the stored host" do
    ...
    navigated = ExecutionRuntime::Browser::SessionManager.call(
      action: "navigate",
      browser_session_id: browser_session_id,
      url: "https://example.com/docs"
    )
    content = ExecutionRuntime::Browser::SessionManager.call(
      action: "get_content",
      browser_session_id: browser_session_id
    )
    screenshot = ExecutionRuntime::Browser::SessionManager.call(
      action: "screenshot",
      browser_session_id: browser_session_id
    )
  ...
  test "list and info expose registered session metadata" do
    ...
    ExecutionRuntime::Browser::SessionManager.call(
      action: "navigate",
      browser_session_id: browser_session_id,
      url: "https://example.com/docs"
    )

    listed = ExecutionRuntime::Browser::SessionManager.call(action: "list")
    info = ExecutionRuntime::Browser::SessionManager.call(action: "info", browser_session_id: browser_session_id)
  ...
  test "list and info can be scoped to the owning execution" do
    ...
    owned = ExecutionRuntime::Browser::SessionManager.call(
      action: "open",
      url: "https://example.com",
      host_factory: host_factory,
    ...
    listed = ExecutionRuntime::Browser::SessionManager.call(action: "list", runtime_owner_id: "task-1")
    info = ExecutionRuntime::Browser::SessionManager.call(
      action: "info",
    ...
    assert_raises(ExecutionRuntime::Browser::SessionManager::ValidationError) do
      ExecutionRuntime::Browser::SessionManager.call(
        action: "info",
        browser_session_id: owned.fetch("browser_session_id"),
        runtime_owner_id: "task-2"
  ...
  test "playwright host wraps broken pipes as host errors" do
    host = ExecutionRuntime::Browser::SessionManager::PlaywrightHost.allocate
    ...
    error = assert_raises(ExecutionRuntime::Browser::SessionManager::HostError) do
  ...
  test "playwright host falls back to a discovered managed browser root when the configured path is empty" do
    Dir.mktmpdir("fenix-playwright-root") do |tmpdir|
      ...
      host = ExecutionRuntime::Browser::SessionManager::PlaywrightHost.new(
