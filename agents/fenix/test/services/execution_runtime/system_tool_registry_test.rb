...
class ExecutionRuntime::SystemToolRegistryTest < ActiveSupport::TestCase
  test "supported tool names cover the command-run and browser executor slices" do
    assert_equal(
      ...
      ExecutionRuntime::SystemToolRegistry.supported_tool_names.sort
    )
  end

  test "registry entries expose executor and catalog metadata" do
    exec_command_entry = ExecutionRuntime::SystemToolRegistry.fetch!("exec_command")

    assert_equal ExecutionRuntime::ToolExecutors::ExecCommand, exec_command_entry.fetch(:executor)
  ...
  test "browser registry entries expose the browser executor slice" do
    browser_open_entry = ExecutionRuntime::SystemToolRegistry.fetch!("browser_open")

    assert_equal ExecutionRuntime::ToolExecutors::Browser, browser_open_entry.fetch(:executor)
  ...
  test "process registry entries expose the detached process executor slice" do
    process_exec_entry = ExecutionRuntime::SystemToolRegistry.fetch!("process_exec")
    process_proxy_info_entry = ExecutionRuntime::SystemToolRegistry.fetch!("process_proxy_info")
