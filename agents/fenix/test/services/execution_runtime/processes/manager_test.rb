...
class FenixProcessesManagerTest < ActiveSupport::TestCase
  ...
  test "spawn! passes the provided environment into the child process" do
    ...
    assert_eventually do
      snapshot = ExecutionRuntime::Processes::Manager.output_snapshot(process_run_id: process_run_id)
  ...
  test "registered processes settle graceful close through close reports" do
    ...
    begin
      ...
      ExecutionRuntime::Processes::Manager.register(
        process_run_id: "process-#{SecureRandom.uuid}",
        runtime_owner_id: "task-1",
        stdin: stdin,
        stdout: stdout,
      ...
      result = ExecutionRuntime::Processes::Manager.close!(
        mailbox_item: {
