...
class Requests::ExecuteToolTest < ActiveSupport::TestCase
  test "executes the shared compact_context contract fixture" do
    ...
    assert_equal "compact_context", response.dig("tool_call", "tool_name")
    assert_equal(
      {
        "messages" => [
          { "role" => "user", "content" => "a" },
  ...
  test "rejects stale calculator tool payloads" do
    payload = {
      "task" => {
        "workflow_node_id" => "workflow-node-public-id",
        "conversation_id" => "conversation-public-id",
