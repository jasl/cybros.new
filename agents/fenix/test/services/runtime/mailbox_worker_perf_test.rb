...
class Runtime::MailboxWorkerPerfTest < ActiveSupport::TestCase
  ...
  test "publishes mailbox execution perf event for inline execution" do
    ...
    Runtime::ExecuteMailboxItem.singleton_class.define_method(:call) do |**kwargs|
      {
        "status" => "ok",
    ...
    Runtime::ExecuteMailboxItem.singleton_class.define_method(:call, original_execute)
  end
  ...
  test "publishes mailbox execution error metadata when inline execution raises" do
    ...
    Runtime::ExecuteMailboxItem.singleton_class.define_method(:call) do |**_kwargs|
      raise "boom"
    end
