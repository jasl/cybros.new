...
class Runtime::RealtimeConnectionTest < ActiveSupport::TestCase
  ...
  test "welcome handshake subscribes and dispatches mailbox payloads" do
    ...
    session = Runtime::RealtimeConnection.new(
    ...
    socket.emit(:message, { "identifier" => Runtime::RealtimeConnection::SUBSCRIPTION_IDENTIFIER, "type" => "confirm_subscription" })
    socket.emit(:message, { "identifier" => Runtime::RealtimeConnection::SUBSCRIPTION_IDENTIFIER, "message" => mailbox_payload("mailbox-item-1") })
    ...
    assert_equal(
      {
        "command" => "subscribe",
        "identifier" => Runtime::RealtimeConnection::SUBSCRIPTION_IDENTIFIER,
      },
