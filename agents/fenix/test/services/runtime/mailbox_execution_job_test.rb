...
class Runtime::MailboxExecutionJobTest < ActiveSupport::TestCase
  setup do
  ...
  test "perform does not require control plane configuration when report delivery is disabled" do
    ...
    result = Runtime::MailboxExecutionJob.perform_now(
      execution_assignment_mailbox_item,
      deliver_reports: false
    )

    assert_equal "ok", result.fetch("status")
    assert_equal "echo", result.dig("output", "kind")
    assert_equal "Echo: deterministic mail.", result.dig("output", "content")
  end
  ...
  test "perform uses explicit control plane context when queued report delivery is enabled" do
    ...
    captured_client = nil
    original_execute = Runtime::ExecuteMailboxItem.method(:call)
    Runtime::ExecuteMailboxItem.singleton_class.define_method(:call) do |mailbox_item:, deliver_reports:, control_client:|
    ...
    assert_equal "ok", result.fetch("status")
    assert_instance_of Shared::ControlPlane::Client, captured_client
  ensure
    Runtime::ExecuteMailboxItem.singleton_class.define_method(:call, original_execute) if original_execute
  end
