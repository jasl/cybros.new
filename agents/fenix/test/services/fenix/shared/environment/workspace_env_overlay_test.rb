...
class Fenix::Shared::Environment::WorkspaceEnvOverlayTest < ActiveSupport::TestCase
  test "returns an empty overlay when the workspace env file is absent" do
    Dir.mktmpdir("fenix-workspace-") do |workspace_root|
      assert_equal({}, Fenix::Shared::Environment::WorkspaceEnvOverlay.call(workspace_root: workspace_root))
    end
  end
  ...
  test "rejects reserved keys" do
    Dir.mktmpdir("fenix-workspace-") do |workspace_root|
      ...
      error = assert_raises(Fenix::Shared::Environment::WorkspaceEnvOverlay::ValidationError) do
        Fenix::Shared::Environment::WorkspaceEnvOverlay.call(workspace_root: workspace_root)
      end
