...
class Fenix::Runtime::RealtimeConnectionTest < ActiveSupport::TestCase
  ...
  test "welcome handshake subscribes and dispatches mailbox payloads" do
    ...
    session = Fenix::Runtime::RealtimeConnection.new(
      base_url: "http://127.0.0.1:3000",
      agent_connection_credential: "machine-credential",
      timeout_seconds: 1,
    ...
    socket.emit(:message, { "identifier" => Fenix::Runtime::RealtimeConnection::SUBSCRIPTION_IDENTIFIER, "type" => "confirm_subscription" })
    socket.emit(:message, { "identifier" => Fenix::Runtime::RealtimeConnection::SUBSCRIPTION_IDENTIFIER, "message" => mailbox_payload("mailbox-item-1") })
    ...
    assert_equal(
      {
        "command" => "subscribe",
        "identifier" => Fenix::Runtime::RealtimeConnection::SUBSCRIPTION_IDENTIFIER,
      },
