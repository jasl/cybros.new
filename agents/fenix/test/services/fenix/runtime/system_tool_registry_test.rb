...
class Fenix::Runtime::SystemToolRegistryTest < ActiveSupport::TestCase
  ...
  test "registry entries expose executor, projector, and catalog metadata" do
  ...
  test "browser registry entries expose the browser executor slice" do
    browser_open_entry = Fenix::Runtime::SystemToolRegistry.fetch!("browser_open")

    assert_equal Fenix::Runtime::ToolExecutors::Browser, browser_open_entry.fetch(:executor)
    assert_equal Fenix::Hooks::ToolResultProjectors::Browser, browser_open_entry.fetch(:projector)
