...
class Fenix::ExecutionRuntime::ToolExecutorTest < ActiveSupport::TestCase
  teardown do
  ...
  test "exec_command rejects invalid workspace env overlays as validation errors" do
    Dir.mktmpdir("fenix-workspace-") do |workspace_root|
      ...
      error = assert_raises(Fenix::ExecutionRuntime::ToolExecutors::ExecCommand::ValidationError) do
        executor.call(
          tool_call: {
      ...
      assert_equal "validation_error", Fenix::ExecutionRuntime::ToolExecutor.error_payload_for(error).fetch("code")
    end
  end
  ...
  test "raises when the tool is not visible for this execution" do
    ...
    error = assert_raises(Fenix::ExecutionRuntime::ToolExecutor::ToolNotAllowedError) do
      executor.call(
        tool_call: {
    ...
    assert_equal "tool_not_allowed", Fenix::ExecutionRuntime::ToolExecutor.error_payload_for(error).fetch("code")
  end
  ...
  test "process_exec rejects invalid workspace env overlays as validation errors" do
    Dir.mktmpdir("fenix-workspace-") do |workspace_root|
      ...
      error = assert_raises(Fenix::ExecutionRuntime::ToolExecutors::Process::ValidationError) do
        executor.call(
          tool_call: {
  ...
  test "maps browser validation failures to semantic error payloads" do
    ...
    error = assert_raises(Fenix::ExecutionRuntime::Browser::SessionManager::ValidationError) do
      with_session_manager_stub(->(**) { raise Fenix::ExecutionRuntime::Browser::SessionManager::ValidationError, "unknown browser session" }) do
        executor.call(
          tool_call: {
  ...
  def with_session_manager_stub(callable)
    session_manager_class = Fenix::ExecutionRuntime::Browser::SessionManager
  ...
  def with_process_runtime_stubs(launcher:, manager_list:, manager_output_snapshot:, proxy_lookup:)
    execution_runtime_module = Fenix::ExecutionRuntime
  ...
  def build_executor(allowed_tool_names:, workspace_root: Dir.pwd)
    Fenix::ExecutionRuntime::ToolExecutor.new(
      context: {
