...
class Fenix::Agent::Requests::ExecuteConversationControlRequestTest < ActiveSupport::TestCase
  test "status refresh returns a structured acknowledgment for the target conversation" do
    response = Fenix::Agent::Requests::ExecuteConversationControlRequest.call(
      payload: {
  ...
  test "active-agent guidance rejects a mismatched subagent target" do
    error = assert_raises(Fenix::Agent::Requests::ExecuteConversationControlRequest::InvalidRequestError) do
      Fenix::Agent::Requests::ExecuteConversationControlRequest.call(
        payload: {
