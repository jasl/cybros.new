...
class Fenix::Operator::CatalogTest < ActiveSupport::TestCase
  test "groups execution tools into operator families" do
    ...
    groups = Fenix::Operator::Catalog.new(tool_catalog: registry.catalog.executor_tool_catalog).groups
  ...
  test "decorates tool entries with operator metadata defaults" do
    entry = Fenix::Plugins::Registry.default.catalog.executor_tool_catalog.find { |tool| tool.fetch("tool_name") == "exec_command" }
