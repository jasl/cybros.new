...
class FenixProcessesProxyRegistryTest < ActiveSupport::TestCase
  teardown do
    Fenix::Executor::Processes::ProxyRegistry.reset_default!
  end
  ...
  test "register writes a stable caddy routes fragment and lookup returns the route" do
    Dir.mktmpdir("fenix-proxy-registry-") do |tmpdir|
      ...
      entry = Fenix::Executor::Processes::ProxyRegistry.new(routes_path: routes_path).register(
        process_run_id: "process-run-1",
        target_port: 4100
      )
      second_entry = Fenix::Executor::Processes::ProxyRegistry.new(routes_path: routes_path).register(
      ...
      assert_equal entry, Fenix::Executor::Processes::ProxyRegistry.new(routes_path: routes_path).lookup(process_run_id: "process-run-1")
      assert_equal second_entry, Fenix::Executor::Processes::ProxyRegistry.new(routes_path: routes_path).lookup(process_run_id: "process-run-2")
