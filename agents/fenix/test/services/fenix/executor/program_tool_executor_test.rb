...
class Fenix::Executor::ProgramToolExecutorTest < ActiveSupport::TestCase
  teardown do
  ...
  test "exec_command rejects invalid workspace env overlays as validation errors" do
    Dir.mktmpdir("fenix-workspace-") do |workspace_root|
      ...
      error = assert_raises(Fenix::Executor::ToolExecutors::ExecCommand::ValidationError) do
        executor.call(
          tool_call: {
      ...
      assert_equal "validation_error", Fenix::Executor::ProgramToolExecutor.error_payload_for(error).fetch("code")
    end
  end
  ...
  test "raises when the tool is not visible for this execution" do
    ...
    error = assert_raises(Fenix::Executor::ProgramToolExecutor::ToolNotAllowedError) do
      executor.call(
        tool_call: {
    ...
    assert_equal "tool_not_allowed", Fenix::Executor::ProgramToolExecutor.error_payload_for(error).fetch("code")
  end
  ...
  test "process_exec rejects invalid workspace env overlays as validation errors" do
    Dir.mktmpdir("fenix-workspace-") do |workspace_root|
      ...
      error = assert_raises(Fenix::Executor::ToolExecutors::Process::ValidationError) do
        executor.call(
          tool_call: {
  ...
  test "maps browser validation failures to semantic error payloads" do
    ...
    error = assert_raises(Fenix::Executor::Browser::SessionManager::ValidationError) do
      with_session_manager_stub(->(**) { raise Fenix::Executor::Browser::SessionManager::ValidationError, "unknown browser session" }) do
        executor.call(
          tool_call: {
  ...
  def with_session_manager_stub(callable)
    session_manager_class = Fenix::Executor::Browser::SessionManager
  ...
  def with_process_runtime_stubs(launcher:, manager_list:, manager_output_snapshot:, proxy_lookup:)
    executor_module_created = false
    processes_module_created = false

    unless Fenix.const_defined?(:Executor, false)
      Fenix.const_set(:Executor, Module.new)
      executor_module_created = true
    end

    executor_module = Fenix.const_get(:Executor)
    unless executor_module.const_defined?(:Processes, false)
      executor_module.const_set(:Processes, Module.new)
      processes_module_created = true
    end

    processes_module = executor_module.const_get(:Processes)
    ...
    if Fenix.const_defined?(:Executor, false) && Fenix::Executor.const_defined?(:Processes, false)
      current_processes_module = Fenix::Executor.const_get(:Processes)
      ...
      Fenix::Executor.send(:remove_const, :Processes) if processes_module_created
    end
    Fenix.send(:remove_const, :Executor) if executor_module_created
  end
