...
class SharedArchitectureBoundariesTest < ActiveSupport::TestCase
  test "service code no longer references the legacy fenix namespace" do
    ...
    offenders = files.filter_map do |path|
      path if File.read(path).match?(/\bFenix::/)
    end

    assert_equal [], offenders
  end

  test "legacy fenix namespace directories are absent from active code" do
    refute_path_exists Rails.root.join("app/services/fenix")
    refute_path_exists Rails.root.join("app/jobs/fenix")
    refute_path_exists Rails.root.join("lib/fenix")
