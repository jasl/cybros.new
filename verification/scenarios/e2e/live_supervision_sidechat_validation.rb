...
bundled = Verification::ManualSupport.register_bundled_runtime_from_manifest!(
  ...
  execution_runtime_fingerprint: "verification-live-supervision-environment",
  fingerprint: "verification-live-supervision-runtime"
)

FileUtils.rm_rf(artifact_dir)
FileUtils.mkdir_p(evidence_dir)

conversation_context = Verification::ManualSupport.create_conversation!(
  agent_definition_version: bundled.agent_definition_version
)
conversation = conversation_context.fetch(:conversation)
turn_workflow = Verification::ManualSupport.start_turn_workflow_on_conversation!(
...
workflow_run.update!(
  ...
  blocking_resource_id: "verification-live-sidechat-blocker"
)
...
diagnostics = Verification::ManualSupport.wait_for_app_api_conversation_diagnostics_materialized!(
  conversation_id: conversation.public_id,
  session_token: app_api_session_token
)
turn_snapshot = diagnostics.fetch("turns").fetch("items").find { |item| item.fetch("turn_id") == turn.public_id }

session_created = Verification::ManualSupport.app_api_create_conversation_supervision_session!(
  conversation_id: conversation.public_id,
...
status_probe = Verification::ManualSupport.app_api_append_conversation_supervision_message!(
  conversation_id: conversation.public_id,
  supervision_session_id: supervision_session_id,
  content: "What are you doing right now and what changed most recently?",
  session_token: app_api_session_token
)
blocker_probe = Verification::ManualSupport.app_api_append_conversation_supervision_message!(
  conversation_id: conversation.public_id,
  supervision_session_id: supervision_session_id,
...
Verification::CapstoneReviewArtifacts.install_live_supervision_sidechat!(
