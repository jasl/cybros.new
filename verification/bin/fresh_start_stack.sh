...
FENIX_HOME_ROOT="${FENIX_HOME_ROOT:-${REPO_ROOT}/tmp/verification-fenix-home}"
...
NEXUS_HOME_ROOT="${NEXUS_HOME_ROOT:-${REPO_ROOT}/tmp/verification-nexus-home}"
...
verification_process_manager_track_process "core-matrix-server" "${CORE_MATRIX_SERVER_PID}" "${CORE_MATRIX_PORT}"
start_core_matrix_jobs_daemon
CORE_MATRIX_JOBS_PID="${STARTED_PID}"
verification_process_manager_track_process "core-matrix-jobs" "${CORE_MATRIX_JOBS_PID}" ""
...
verification_process_manager_track_process "fenix-runtime-server" "${FENIX_RUNTIME_PID}" "${FENIX_RUNTIME_PORT}"
if [[ "${FENIX_HOST_START_JOBS_DAEMON}" == "true" ]]; then
  start_fenix_jobs_daemon
  FENIX_RUNTIME_JOBS_PID="${STARTED_PID}"
  verification_process_manager_track_process "fenix-runtime-jobs" "${FENIX_RUNTIME_JOBS_PID}" ""
else
