...
require "verification/suites/e2e/manual_support"
require "tmpdir"
require "zip"

class Verification::ManualSupportTest < ActiveSupport::TestCase
  ...
  test "execute_provider_workflow! uses a provider-backed timeout that fits real verification runs" do
    ...
    with_redefined_singleton_method(Workflows::ExecuteRun, :call, ->(*) { nil }) do
      with_redefined_singleton_method(
        ...
        Verification::ManualSupport.execute_provider_workflow!(workflow_run:)
      end
    end
  ...
  test "execute_provider_workflow! still honors an explicit timeout override" do
    ...
    with_redefined_singleton_method(Workflows::ExecuteRun, :call, ->(*) { nil }) do
      with_redefined_singleton_method(
        ...
        Verification::ManualSupport.execute_provider_workflow!(workflow_run:, timeout_seconds: 42)
      end
    end
  ...
  test "execute_provider_turn_on_conversation! forwards catalog overrides into provider execution" do
    ...
    with_redefined_singleton_method(
      ...
      with_redefined_singleton_method(
        ...
        result = Verification::ManualSupport.execute_provider_turn_on_conversation!(
          conversation: conversation,
  ...
  test "execute_tool_call! retries deferred mailbox exchanges until a terminal result arrives" do
    ...
    with_redefined_singleton_method(
      ...
      returned = Verification::ManualSupport.execute_tool_call!(
    ...
    with_redefined_singleton_method(Workflows::ExecuteRun, :call, ->(*) { dispatched_node }) do
      with_redefined_singleton_method(
        ...
        with_redefined_singleton_method(
          ...
          Verification::ManualSupport.execute_provider_workflow!(
            workflow_run: workflow_run,
            inline_if_queued: false
          )
        end
  ...
  test "wait_for_workflow_run_terminal! keeps draining queued nodes when inline execution is enabled" do
    ...
    with_redefined_singleton_method(
      ...
      with_redefined_singleton_method(
        ...
        result = Verification::ManualSupport.wait_for_workflow_run_terminal!(
          workflow_run: workflow_run,
          timeout_seconds: 1,
          poll_interval_seconds: 0.0,
  ...
  test "execute_inline_if_queued! forwards catalog overrides into execute node" do
    ...
    with_redefined_singleton_method(WorkflowNode, :find_by, ->(public_id:) { current_node }) do
      with_redefined_singleton_method(
        ...
        Verification::ManualSupport.execute_inline_if_queued!(
  ...
  test "download_public_url! follows redirects before writing the destination file" do
    ...
    with_redefined_singleton_method(
      ...
      download = Verification::ManualSupport.download_public_url!(
  ...
  test "reset_backend_state! disconnects, rebuilds, and reconnects the database" do
    ...
    with_redefined_singleton_method(Verification::ManualSupport, :disconnect_application_record!, -> { calls << :disconnect }) do
      with_redefined_singleton_method(Verification::ManualSupport, :run_database_reset_command!, -> { calls << :reset }) do
        with_redefined_singleton_method(Verification::ManualSupport, :reconnect_application_record!, -> { calls << :reconnect }) do
  ...
  test "reset_backend_state! surfaces the database reset failure before reconnecting" do
    ...
    with_redefined_singleton_method(Verification::ManualSupport, :disconnect_application_record!, -> { calls << :disconnect }) do
      with_redefined_singleton_method(Verification::ManualSupport, :run_database_reset_command!, lambda {
        calls << :reset
        raise error
      }) do
        with_redefined_singleton_method(Verification::ManualSupport, :reconnect_application_record!, -> { calls << :reconnect }) do
          raised = assert_raises(RuntimeError) { Verification::ManualSupport.reset_backend_state! }
  ...
  test "run_database_reset_command! rebuilds the schema from migrations before db:reset" do
    ...
    with_redefined_singleton_method(Bundler, :with_unbundled_env, ->(&block) { block.call }) do
      with_redefined_singleton_method(Open3, :capture3, lambda { |*args, **kwargs|
        ...
        result = Verification::ManualSupport.run_database_reset_command!
  ...
  test "run_fenix_runtime_task! forwards FENIX_HOME_ROOT into the spawned fenix task" do
    ...
    ENV["FENIX_HOME_ROOT"] = "/tmp/verification-fenix-home"
    ...
    with_redefined_singleton_method(Bundler, :with_unbundled_env, ->(&block) { block.call }) do
      with_redefined_singleton_method(Open3, :capture3, lambda { |*args, **kwargs|
        ...
        with_redefined_singleton_method(
          ...
          result = Verification::ManualSupport.run_fenix_runtime_task!(
    ...
    assert_equal "/tmp/verification-fenix-home", env.fetch("FENIX_HOME_ROOT")
    ...
    with_redefined_singleton_method(Bundler, :with_unbundled_env, ->(&block) { block.call }) do
      with_redefined_singleton_method(Open3, :capture3, lambda { |*args, **kwargs|
        ...
        with_redefined_singleton_method(
          ...
          Verification::ManualSupport.run_fenix_runtime_task!(
            task_name: "runtime:control_loop_once",
            agent_connection_credential: "agent-secret",
            execution_runtime_connection_credential: "execution-secret",
            env: {
  ...
  test "with_fenix_control_worker! lets explicit runtime env override the forwarded fenix home root" do
    ...
    assert_equal "/tmp/fenix-slot-events.ndjson", env.fetch("CYBROS_PERF_EVENTS_PATH")
    assert_equal "fenix-03", env.fetch("CYBROS_PERF_INSTANCE_LABEL")
    assert_equal({ pid: 43_210, timeout_seconds: 30, worker_label: "fenix control worker" }, wait_args)
  ensure
    ENV["FENIX_HOME_ROOT"] = previous_home_root
  end

  test "wait_for_worker_ready! accepts ready json even when log noise prefixes the line" do
    reader, writer = IO.pipe
    writer_thread = nil

    begin
      writer_thread = Thread.new do
        writer.write("booting runtime...")
        writer.write(%({"event":"ready","pid":1234}\n))
        writer.flush
        writer.close
      end

      payload = Verification::ManualSupport.wait_for_worker_ready!(
        reader: reader,
        pid: Process.pid,
        timeout_seconds: 0.1,
      ...
      assert_equal({ "event" => "ready", "pid" => 1234 }, payload)
    ensure
      writer_thread&.join
  ...
  test "wait_for_worker_ready! reports the worker label and recent output on timeout" do
    reader, writer = IO.pipe
    writer.puts("booting runtime")
    writer.flush

    error = assert_raises(RuntimeError) do
      Verification::ManualSupport.wait_for_worker_ready!(
        reader: reader,
        pid: Process.pid,
    ...
    assert_includes error.message, "nexus control worker"
    assert_includes error.message, "booting runtime"
  ensure
    writer.close unless writer.closed?
    reader.close unless reader.closed?
  end
  ...
  test "run_fenix_control_loop_for_registration! forwards executor credentials from runtime registration" do
    captured = nil
    registration = Verification::ManualSupport::RuntimeRegistration.new(
      manifest: {},
      agent_connection_credential: "agent-secret",
      execution_runtime_connection_credential: "execution-secret",
    ...
    with_redefined_singleton_method(
      ...
      result = Verification::ManualSupport.run_fenix_control_loop_for_registration!(
  ...
  test "with_fenix_control_worker_for_registration! falls back to the agent credential when executor credential is absent" do
    ...
    with_redefined_singleton_method(
      ...
      Verification::ManualSupport.with_fenix_control_worker_for_registration!(
        registration: registration,
        limit: 2
      ) do |pid|
        yielded = pid
  ...
  test "reconnect_application_record! re-establishes and checks out through with_connection" do
    ...
    with_redefined_singleton_method(ApplicationRecord, :establish_connection, -> { calls << :establish }) do
      with_redefined_singleton_method(ApplicationRecord, :with_connection, lambda { |&block|
        ...
        Verification::ManualSupport.reconnect_application_record!
      end
    end
  ...
  test "register_bring_your_own_runtime! returns typed version and connection artifacts from the registration payloads" do
    ...
    execution_runtime_connection = Struct.new(:public_id).new("rtc_123")
      ...
      with_redefined_singleton_method(Verification::ManualSupport, :live_manifest, ->(base_url:) { manifest }) do
      with_redefined_singleton_method(Verification::ManualSupport, :issue_app_api_session_token!, ->(user:, expires_at: 30.days.from_now) { "session-secret" }) do
        with_redefined_singleton_method(
          Verification::ManualSupport,
              ...
              with_redefined_singleton_method(
                ...
                with_redefined_singleton_method(
                  ...
                  with_redefined_singleton_method(
                    ...
                    result = Verification::ManualSupport.register_bring_your_own_runtime!(
                      onboarding_token: "onboarding-token",
                      runtime_base_url: "http://127.0.0.1:3101",
                      execution_runtime_fingerprint: "runtime-fingerprint"
                    )

                    assert_instance_of Verification::ManualSupport::RuntimeRegistration, result
    ...
    with_redefined_singleton_method(Verification::ManualSupport, :live_manifest, ->(base_url:) { manifest }) do
      with_redefined_singleton_method(
        ...
        with_redefined_singleton_method(AgentDefinitionVersion, :find_by_public_id!, ->(public_id) { public_id == "adv_456" ? agent_definition_version : nil }) do
          ...
          result = Verification::ManualSupport.register_bring_your_own_agent_from_manifest!(
            onboarding_token: "onboarding-token",
    ...
    with_redefined_singleton_method(Verification::ManualSupport, :live_manifest, ->(base_url:) { manifest }) do
      with_redefined_singleton_method(
        ...
        with_redefined_singleton_method(ExecutionRuntime, :find_by_public_id!, ->(public_id) { public_id == "rt_456" ? execution_runtime : nil }) do
          ...
          result = Verification::ManualSupport.register_bring_your_own_execution_runtime!(
            onboarding_token: "onboarding-token",
  ...
  test "manual support exposes bring-your-own registration helpers instead of removed external helper names" do
    ...
    assert_respond_to Verification::ManualSupport, :create_bring_your_own_agent!
    assert_respond_to Verification::ManualSupport, :create_bring_your_own_execution_runtime!
    assert_respond_to Verification::ManualSupport, :register_bring_your_own_runtime!
    assert_respond_to Verification::ManualSupport, :register_bring_your_own_agent_from_manifest!
    assert_respond_to Verification::ManualSupport, :register_bring_your_own_execution_runtime!
    refute_respond_to Verification::ManualSupport, removed_create_helper
    refute_respond_to Verification::ManualSupport, removed_register_runtime_helper
    refute_respond_to Verification::ManualSupport, removed_register_execution_runtime_helper
  end
    ...
    with_redefined_singleton_method(Verification::ManualSupport, :issue_app_api_session_token!, ->(user:, expires_at: 30.days.from_now) { "session-secret" }) do
      with_redefined_singleton_method(
        ...
        with_redefined_singleton_method(OnboardingSession, :find_by_public_id!, lambda { |public_id|
          ...
          result = Verification::ManualSupport.create_bring_your_own_execution_runtime!(
      ...
      with_redefined_singleton_method(
        ...
        with_redefined_singleton_method(OnboardingSession, :find_by_public_id!, ->(public_id) { public_id == "ons_123" ? onboarding_session : nil }) do
          with_redefined_singleton_method(Agent, :find_by_public_id!, ->(public_id) { public_id == "agt_123" ? agent : nil }) do
            result = Verification::ManualSupport.create_bring_your_own_agent!(
              installation: "installation",
              actor: "actor",
  ...
  test "register_bundled_runtime_from_manifest! preserves explicit executor connection metadata from the manifest" do
    ...
    with_redefined_singleton_method(Verification::ManualSupport, :live_manifest, ->(base_url:) { manifest }) do
      with_redefined_singleton_method(
        ...
        result = Verification::ManualSupport.register_bundled_runtime_from_manifest!(
          installation: "installation",
          runtime_base_url: "http://127.0.0.1:3101",
          execution_runtime_fingerprint: "runtime-fingerprint",
  ...
  test "register_bundled_runtime_from_manifest! falls back to the runtime base url when executor connection metadata is omitted" do
    ...
    with_redefined_singleton_method(Verification::ManualSupport, :live_manifest, ->(base_url:) { manifest }) do
      with_redefined_singleton_method(
        ...
        Verification::ManualSupport.register_bundled_runtime_from_manifest!(
          installation: "installation",
          runtime_base_url: "http://127.0.0.1:3101",
          execution_runtime_fingerprint: "runtime-fingerprint",
          fingerprint: "agent-fingerprint"
      ...
      with_redefined_singleton_method(
        ...
        with_redefined_singleton_method(
          ...
          with_redefined_singleton_method(
            ...
            with_redefined_singleton_method(Verification::ManualSupport, :report_results_for, ->(agent_task_run:) { [] }) do
              result = Verification::ManualSupport.run_fenix_mailbox_task!(
  ...
  test "create_conversation_supervision_session! calls the create-session service and serializes the result" do
    ...
    with_redefined_singleton_method(
      ...
      with_redefined_singleton_method(
        ...
        result = Verification::ManualSupport.create_conversation_supervision_session!(
  ...
  test "append_conversation_supervision_message! calls the append-message service and serializes the exchange" do
    ...
    with_redefined_singleton_method(
      ...
      with_redefined_singleton_method(
        ...
        result = Verification::ManualSupport.append_conversation_supervision_message!(
  ...
  test "app_api_create_conversation! forwards execution runtime overrides to app_api" do
    ...
    with_redefined_singleton_method(
      ...
      result = Verification::ManualSupport.app_api_create_conversation!(
  ...
  test "execution_runtime_publish_output_attachment! posts multipart data to the runtime attachment publish route" do
    ...
    with_redefined_singleton_method(
      ...
      result = Verification::ManualSupport.execution_runtime_publish_output_attachment!(
        turn_id: "turn_123",
  ...
  test "app_api_create_conversation_supervision_session! posts to the nested supervision session route" do
    ...
    with_redefined_singleton_method(
      ...
      result = Verification::ManualSupport.app_api_create_conversation_supervision_session!(
        conversation_id: "conv_123",
        responder_strategy: "builtin",
  ...
  test "app_api_append_conversation_supervision_message! posts to the nested supervision message route" do
    ...
    with_redefined_singleton_method(
      ...
      result = Verification::ManualSupport.app_api_append_conversation_supervision_message!(
        conversation_id: "conv_123",
        supervision_session_id: "session_123",
  ...
  test "app_api_append_conversation_supervision_message_with_retry! retries once when the first summary reply refuses" do
    ...
    with_redefined_singleton_method(
      ...
      result = Verification::ManualSupport.app_api_append_conversation_supervision_message_with_retry!(
        conversation_id: "conv_123",
        supervision_session_id: "session_123",
  ...
  test "app_api_conversation_supervision_messages! gets the nested supervision message list route" do
    ...
    with_redefined_singleton_method(
      ...
      result = Verification::ManualSupport.app_api_conversation_supervision_messages!(
        conversation_id: "conv_123",
        supervision_session_id: "session_123",
        session_token: "session-token"
      )
  ...
  test "reset_backend_state! is a no-op when verification skip env is enabled" do
    calls = []
    previous = ENV["VERIFICATION_SKIP_BACKEND_RESET"]
    ENV["VERIFICATION_SKIP_BACKEND_RESET"] = "true"
    ...
    ENV["VERIFICATION_SKIP_BACKEND_RESET"] = previous
  end
  ...
  test "wait_for_app_api_turn_terminal! polls diagnostics until the target turn reaches a terminal state" do
    ...
    with_redefined_singleton_method(
      ...
      with_redefined_singleton_method(
        ...
        payload = Verification::ManualSupport.wait_for_app_api_turn_terminal!(
          conversation_id: "conv_123",
          turn_id: "turn_123",
          session_token: "session-token",
          timeout_seconds: 1,
    ...
    with_redefined_singleton_method(
      ...
      with_redefined_singleton_method(
        ...
        with_redefined_singleton_method(
          ...
          payload = Verification::ManualSupport.wait_for_app_api_turn_live_activity!(
            conversation_id: "conv_123",
            turn_id: "turn_123",
            session_token: "session-token",
            timeout_seconds: 1,
  ...
  test "wait_for_app_api_turn_live_activity! fails if the turn finishes before live activity is observed" do
    ...
    with_redefined_singleton_method(
      ...
      error = assert_raises(RuntimeError) do
        Verification::ManualSupport.wait_for_app_api_turn_live_activity!(
          conversation_id: "conv_123",
          turn_id: "turn_123",
          session_token: "session-token",
          timeout_seconds: 1,
  ...
  test "turn_live_activity_metrics summarizes the live progress counters" do
    metrics = Verification::ManualSupport.turn_live_activity_metrics(
      turn: {
  ...
  test "app_api_conversation_transcript! uses the nested transcript route" do
    ...
    with_redefined_singleton_method(
      ...
      Verification::ManualSupport.app_api_conversation_transcript!(
        conversation_id: "conv_123",
        session_token: "session-token",
        cursor: "msg_123",
        limit: 5
  ...
  test "app_api_conversation_diagnostics helpers use nested diagnostics routes" do
    ...
    with_redefined_singleton_method(
      ...
      Verification::ManualSupport.app_api_conversation_diagnostics_show!(
        conversation_id: "conv_123",
        session_token: "session-token"
      )
      Verification::ManualSupport.app_api_conversation_diagnostics_turns!(
        conversation_id: "conv_123",
        session_token: "session-token"
      )
    end
  ...
  test "app_api conversation feed and runtime event helpers use scoped routes" do
    ...
    with_redefined_singleton_method(
      ...
      Verification::ManualSupport.app_api_conversation_feed!(
        conversation_id: "conv_123",
        session_token: "session-token"
      )
      Verification::ManualSupport.app_api_conversation_turn_runtime_events!(
        conversation_id: "conv_123",
        turn_id: "turn_123",
        session_token: "session-token"
      )
  ...
  test "extract_debug_export_payload! reads the canonical debug export json members" do
    Tempfile.create(["conversation-debug-export", ".zip"]) do |tempfile|
      Zip::OutputStream.open(tempfile.path) do |zip|
        ...
        zip.put_next_entry("workflow_edges.json")
        zip.write(JSON.generate([{ "from_node_key" => "turn_step", "to_node_key" => "provider_round_1_tool_1" }]))
        zip.put_next_entry("workflow_artifacts.json")
        zip.write(JSON.generate([{ "artifact_key" => "batch-1:stage:0", "barrier_kind" => "wait_all" }]))
      ...
      payload = Verification::ManualSupport.extract_debug_export_payload!(tempfile.path)
      ...
      assert_equal [{ "from_node_key" => "turn_step", "to_node_key" => "provider_round_1_tool_1" }], payload.fetch("workflow_edges")
      assert_equal [{ "artifact_key" => "batch-1:stage:0", "barrier_kind" => "wait_all" }], payload.fetch("workflow_artifacts")
