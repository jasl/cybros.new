...
module Verification
  module HostValidation
    ...
    NEXUS_VERSIONS_ENV_PATH = Verification::Adapters::Nexus.root.join("versions.env").to_s
    ...
    def run!(generated_app_dir:, artifact_dir:, preview_port:, runtime_validation:, persist_artifacts: true, phase_logger: nil)
      ...
      if generated_app_dir.exist?
        ...
        if dist_dir.join("index.html").exist?
          begin
    ...
    def playability_failure_observations(playwright_validation:)
      ...
      observations << "- host browser verification never observed a new tile spawn after a valid move." if result["spawnObserved"] == false
      observations << "- host browser play filled the board without observing a visible `Game over` status." if result["gameOverReached"] == false
      observations << "- restart did not reset the score to `0`." if result["restartResetScore"] == false
      observations << "- restart did not reset the board back to exactly two starting tiles." if result["restartResetTileCount"] == false
      observations
    end
    ...
    def write_playability_verification!(path:, playability_result:, playwright_test:, generated_app_dir:, preview_port:, runtime_validation:, preview_validation:, host_skip_reason: nil)
      ...
      unless playwright_test&.fetch("success", false)
        lines = [
          ...
          "",
          "Playwright captured a real browser session and wrote `playable/host-playwright-verification.json`, but one or more verification assertions failed.",
          "",
    ...
    def build_host_preview_failure_message(error:, preview_pid:, preview_log:, preview_port:)
      ...
      [error.message, process_details, (log_excerpt ? "preview log:\n#{log_excerpt}" : nil)].compact.join("\n")
    end
    ...
    def build_playwright_script(output_json_path:, screenshot_path:)
      <<~JAVASCRIPT
        ...
        async function boardCellTexts(page) {
          ...
          if ((await cells.count()) === 0) cells = page.locator('[data-testid^="tile-"]');
        ...
        async function waitForChange(page, previous) {
          ...
          try {
            await page.waitForFunction((prior) => {
              ...
              return JSON.stringify({ board: rows, score, status, nonEmpty: flat.filter((value) => value !== null && value !== 0).length }) !== prior;
        ...
        async function verifyDirectionFromFreshBoard(page, key, maxAttempts = 20) {
          for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
            ...
            return { changed: true, beforeScore: before.score, afterScore: after.score, attempts: attempt + 1 };
          }
          return { changed: false, beforeScore: null, afterScore: null, attempts: maxAttempts };
        }
        ...
        test('host-side 2048 playability', async ({ page }) => {
          ...
          for (let step = 0; step < 1500; step += 1) {
            ...
            for (const key of priority) {
              ...
              if (!changed) continue;
              const after = await snapshot(page);
              directionChecks[key] = { changed: true, beforeScore: before.score, afterScore: after.score };
    ...
    def write_validation_artifacts(artifact_dir:, generated_app_dir:, preview_port:, host_validation:, runtime_validation:, playwright_validation:, host_playability_skip_reason:)
      write_json(artifact_dir.join("evidence", "host-validation.json"), host_validation)
      write_json(artifact_dir.join("evidence", "runtime-validation.json"), runtime_validation)
      write_json(artifact_dir.join("playable", "host-playwright-install.json"), playwright_validation.fetch("install")) if playwright_validation.any?
      write_json(artifact_dir.join("playable", "host-playwright-test.json"), playwright_validation.fetch("test")) if playwright_validation.any?
      write_json(artifact_dir.join("playable", "host-playwright-verification.json"), playwright_validation.fetch("result")) if playwright_result_available?(playwright_validation)

      write_text(artifact_dir.join("review", "workspace-validation.md"), <<~MD)
        # Workspace Validation

        - `npm install` success: `#{host_validation.dig("npm_install", "success")}`
        - `npm test` success: `#{host_validation.dig("npm_test", "success")}`
        - `npm run build` success: `#{host_validation.dig("npm_build", "success")}`
        - host preview reachable: `#{host_validation.dig("preview_http", "status") == 200}`
      ...
      write_playability_verification!(
        ...
        runtime_validation: runtime_validation,
        preview_validation: {
          "reachable" => host_validation.dig("preview_http", "status") == 200,
          "content_excerpt" => host_validation.dig("preview_http", "content_excerpt"),
        },
        host_skip_reason: host_playability_skip_reason
      )
    end
    private_class_method :write_validation_artifacts
